...
func test_exploration_and_dot_follow_scale() -> void:
	...
	assert_eq(BalanceConfig.DOT_FLAT_POISON, 2 * BalanceConfig.STAT_SCALE)
	assert_eq(BalanceConfig.DOT_FLAT_IGNITE, 3 * BalanceConfig.STAT_SCALE / 2)
	...
	var bleed: Resource = DataRegistry.get_status_effect("bleed")
	assert_not_null(poison)
	assert_not_null(ignite)
	assert_not_null(bleed)
	assert_eq(int(poison.dot_flat), BalanceConfig.DOT_FLAT_POISON)
	assert_eq(int(ignite.dot_flat), BalanceConfig.DOT_FLAT_IGNITE)
	assert_almost_eq(float(poison.dot_percent_of_attack), BalanceConfig.DOT_PCT_POISON, 0.0001)
	assert_almost_eq(float(ignite.dot_percent_of_attack), BalanceConfig.DOT_PCT_IGNITE, 0.0001)
	assert_almost_eq(float(bleed.dot_percent_of_attack), BalanceConfig.DOT_PCT_BLEED, 0.0001)
	assert_eq(int(poison.duration_ticks), BalanceConfig.DOT_TICKS_POISON)
	assert_eq(int(ignite.duration_ticks), BalanceConfig.DOT_TICKS_IGNITE)
	assert_eq(int(bleed.duration_ticks), BalanceConfig.DOT_TICKS_BLEED)
