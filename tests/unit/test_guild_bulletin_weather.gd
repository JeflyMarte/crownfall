...
func test_weather_effect_bullets_match_combat_math() -> void:
	...
	assert_true("回復" in "\n".join(fog))
	assert_true("毒" in "\n".join(fog))
	assert_eq(CombatWeather.outgoing_multiplier(CombatWeather.FOG), 1.0)
	assert_eq(CombatWeather.incoming_multiplier(CombatWeather.FOG), 1.0)
	assert_eq(CombatWeather.heal_received_multiplier(CombatWeather.FOG), 0.85)
	assert_eq(CombatWeather.poison_damage_multiplier(CombatWeather.FOG), 1.15)
...
func test_weather_effect_one_line_for_legend() -> void:
	...
	assert_eq(CombatWeather.effect_one_line(CombatWeather.RAIN), "雨:雷+10%／炎−5%")
	assert_eq(CombatWeather.effect_one_line(CombatWeather.FOG), "霧:回復−15%／毒+15%")
	assert_eq(CombatWeather.effect_one_line(CombatWeather.SNOW), "吹雪:氷+10%／炎−5%")
...
func test_weather_slot_shows_active_weather_combat() -> void:
	...
	assert_true("回復" in guide, guide)
	assert_true("毒" in guide, guide)
