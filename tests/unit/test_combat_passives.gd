...
func test_starter_and_gacha_passive_redesign() -> void:
	...
	assert_false(elias.has("heal_max_hp_fraction"))
	assert_false(elias.has("trigger"))
	...
	assert_eq(str(garm.get("display_name", "")), "不屈の鼓動")
	assert_almost_eq(float(garm.get("combat_regen_interval_sec", 0.0)), 3.0, 0.001)
	assert_almost_eq(float(garm.get("combat_regen_max_hp_fraction", 0.0)), 0.02, 0.001)
	assert_almost_eq(float(garm.get("threat_base_add", 0.0)), 70.0, 0.001)
	assert_false(garm.has("death_save_chance"))
	...
	assert_eq(str(mira.get("display_name", "")), "血契の矢")
	assert_almost_eq(float(mira.get("lifesteal_ratio", 0.0)), 0.15, 0.001)
	assert_true(bool(mira.get("lifesteal_basic_only", false)))
	assert_false(mira.has("status_id"))
	assert_false(mira.has("pet_heal_on_action_max_hp_fraction"))
