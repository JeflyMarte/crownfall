...
func test_phase_a_second_skills_complement_first() -> void:
	...
	assert_true(frog.skill_ids.has("enemy_mire_miasma"))
	assert_true(frog.skill_ids.has("enemy_mire_toxin_bloom"))
	var bloom: Resource = DataRegistry.get_skill_data("enemy_mire_toxin_bloom")
	assert_eq(str(bloom.target_type), "all_party")
	assert_eq(str(bloom.apply_status_id), "mire_toxin")
	var joe: Resource = DataRegistry.get_enemy_data("storm_joe")
	assert_true(joe.skill_ids.has("enemy_blizzard_howl"))
	assert_true(joe.skill_ids.has("enemy_gale_silence"))
	var gale: Resource = DataRegistry.get_skill_data("enemy_gale_silence")
	assert_eq(str(gale.element), "ice")
	assert_eq(str(gale.effect_type), "silence")
