...
func test_shadow_stalker_high_risk_loot() -> void:
	...
	assert_eq(int(data.max_hp), 2000)
	assert_eq(int(data.attack), 295)
	assert_eq(str(data.skill_ids[0]), "enemy_shadow_reap")
	assert_almost_eq(float(data.skill_use_chance), 0.4, 0.0001)
	assert_almost_eq(float(data.on_hit_status_chance), 0.3, 0.0001)
	...
	assert_almost_eq(float(skill.power_multiplier), 1.65, 0.001)
