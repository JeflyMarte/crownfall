...
func test_dungeon_weather_and_band_gate_light_mode() -> void:
	...
	assert_true(src.contains("_start_party_card_critical_pulse"), "party critical pulse present")
	assert_true(src.contains("_set_skill_cd_ready_pulse"), "skill ready pulse present")
