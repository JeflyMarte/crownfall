...
func test_star_shards_progress_table() -> void:
	assert_eq(_RoyalMarkConfig.star_shards_for_progress(0, 1), 3)
	assert_eq(_RoyalMarkConfig.star_shards_for_progress(1, 3), 7)
	assert_eq(_RoyalMarkConfig.star_shards_for_progress(0, 4), 15)
	assert_eq(_RoyalMarkConfig.star_shards_for_progress(4, 4), 0)
	assert_eq(_RoyalMarkConfig.retroactive_shards_for_best(0), 0)
	assert_eq(_RoyalMarkConfig.retroactive_shards_for_best(1), 3)
	assert_eq(_RoyalMarkConfig.retroactive_shards_for_best(2), 6)
	assert_eq(_RoyalMarkConfig.retroactive_shards_for_best(3), 10)
	assert_eq(_RoyalMarkConfig.retroactive_shards_for_best(4), 15)
...
func test_v17_migration_retroactive_star1() -> void:
	...
	data = SaveManager._migrate_save_data(data)
	assert_eq(int(data.get("royal_mark_shards", 0)), 3)
...
func test_v17_migration_retroactive_star4() -> void:
	...
	data = SaveManager._migrate_save_data(data)
	assert_eq(int(data.get("royal_mark_shards", 0)), 15)
...
func test_shards_insufficient() -> void:
	...
	var m: Resource = _make_human("adventurer_0", 50)
	GameState.roster = [m]
	GameState.royal_mark_shards = 9
	GameState.gold = 5000
	var check: Dictionary = _RoyalMarkSystem.can_upgrade(m)
	assert_false(bool(check.get("ok", false)))
	assert_eq(str(check.get("reason", "")), "need_shards")
	assert_eq(_RoyalMarkSystem.get_shards(), 9)
...
func test_star_reward_first_star1() -> void:
	...
	assert_eq(int(g.get("star_shards", 0)), 3)
	assert_eq(int(g.get("repeat_shards", 0)), 0)
	assert_eq(_RoyalMarkSystem.get_shards(), 3)
...
func test_star_reward_1_to_3() -> void:
	...
	assert_eq(int(g.get("star_shards", 0)), 7)
	assert_eq(_RoyalMarkSystem.get_shards(), 7)
...
func test_star_reward_first_star4() -> void:
	...
	assert_eq(int(g.get("star_shards", 0)), 15)
	assert_eq(_RoyalMarkSystem.get_shards(), 15)
...
func test_repeat_hit() -> void:
	...
	assert_eq(int(g.get("repeat_shards", 0)), 1)
	assert_eq(_RoyalMarkSystem.get_shards(), 1)
...
func test_double_commit_no_extra_shards_or_reroll() -> void:
	...
	assert_eq(int(first.get("star_shards", 0)), 15)
	assert_eq(int(first.get("repeat_shards", 0)), 1)
	assert_eq(_RoyalMarkSystem.get_shards(), 16)
	## 二重: repeat force を再度 hit にしても増えない・再抽選なし
	var second: Dictionary = _RoyalMarkSystem.grant_extreme_clear_rewards(
		"ex_tomb_seal", 0, 4, null, 0.0
	)
	assert_true(bool(second.get("skipped_duplicate", false)))
	assert_eq(_RoyalMarkSystem.get_shards(), 16)
	assert_eq(int(GameState.last_run_royal_mark_shards_total), 16)
	assert_eq(int(second.get("star_shards", 0)), 15)
	assert_eq(int(second.get("repeat_shards", 0)), 1)
...
func test_commit_clear_result_wires_royal_mark() -> void:
	...
	assert_eq(int(GameState.last_run_royal_mark_shards_star), 3)
	...
	assert_eq(_RoyalMarkSystem.get_shards(), 3)
	assert_eq(int(GameState.last_run_royal_mark_shards_total), 3)
