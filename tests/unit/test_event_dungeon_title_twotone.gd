...
func test_split_bracket_descent_and_conquest_titles() -> void:
	var chronos: Dictionary = _Helper.split_title("【時環の共鳴龍クロノス・ウェーブ】降臨")
	assert_eq(str(chronos.get("body", "")), "【時環の共鳴龍クロノス・ウェーブ】")
	assert_eq(str(chronos.get("suffix", "")), "降臨")
	var valgard: Dictionary = _Helper.split_title("【境界の番ヴァルガード】降臨")
	assert_eq(str(valgard.get("body", "")), "【境界の番ヴァルガード】")
	assert_eq(str(valgard.get("suffix", "")), "降臨")
	var conquest: Dictionary = _Helper.split_title("【地図なき主アルバーク】征討")
	assert_eq(str(conquest.get("body", "")), "【地図なき主アルバーク】")
...
func test_route_twotone_and_body_colors() -> void:
	assert_true(_Helper.is_route_twotone("【時環の共鳴龍クロノス・ウェーブ】降臨"))
	assert_true(_Helper.is_route_twotone("【星炉の寝主フォージ・ドルミエント】征討"))
...
func test_title_bbcode_twotone_and_plain() -> void:
	var bb: String = _Helper.title_bbcode(
		"chronos_mausoleum", "【時環の共鳴龍クロノス・ウェーブ】降臨", true
	)
	assert_true("降臨" in bb)
	assert_true("【時環の共鳴龍クロノス・ウェーブ】" in bb)
	assert_true("[color=#" in bb)
	var conquest: String = _Helper.title_bbcode(
		"north_reach", "【地図なき主アルバーク】征討", true
	)
