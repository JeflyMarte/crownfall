...
func test_passive_effects_match_brief() -> void:
	...
	assert_almost_eq(float(spine["element_outgoing_mult"]["ice"]), 1.10, 0.001)
	assert_almost_eq(float(spine.get("back_row_basic_attack_mult", 1.0)), 1.25, 0.001)
	assert_almost_eq(float(spine.get("outgoing_vs_status_mult", 1.0)), 1.20, 0.001)
	...
	assert_almost_eq(float(claw.get("outgoing_vs_status_mult", 1.0)), 1.30, 0.001)
