...
func test_guide_no_outdated_combat_or_gacha_copy() -> void:
	...
	assert_true(
		str(by_id.get("SYS-G015", "")).contains("いつでも")
		or str(by_id.get("SYS-G015", "")).contains("常設"),
		"極限常設"
	)
...
func test_hub_and_field_guide_entries_exist() -> void:
	...
	assert_true(event_dg.contains("エンシェント") or event_dg.contains("20"), "征討の中身に言及")
	assert_false(event_dg.contains("地図なき"), "固有名は出さない")
	assert_false(event_dg.contains("名拒み"), "セット名は出さない")
...
func test_world_guide_entries_cover_canon_basics() -> void:
	...
	assert_true(str(by_id.get("WORLD-G041", "")).contains("20") or str(by_id.get("WORLD-G041", "")).contains("エンシェント"), "征討手引きに常設長編を記載")
	assert_false(str(by_id.get("WORLD-G041", "")).contains("寄り道・征討は選択できない"), "征討選択不可の旧文を撤去")
	assert_false(str(by_id.get("WORLD-G041", "")).contains("地図なき"), "固有名は出さない")
	assert_false(str(by_id.get("WORLD-G041", "")).contains("名拒み"), "セット名は出さない")
	assert_true(str(by_id.get("WORLD-G042", "")).contains("寄り道は選択できない"), "寄り道オミットは維持")
