...
func test_boss_material_loot_grants_one_on_normal() -> void:
	...
	assert_eq(str(bonus.get("material_id", "")), EquipmentEnhancer.LEGEND_ORE_ID)
	assert_eq(int(bonus.get("amount", 0)), 1)
	assert_eq(GameState.get_material_quantity(EquipmentEnhancer.LEGEND_ORE_ID), 1)
...
func test_boss_material_loot_grants_two_on_hard() -> void:
	...
	assert_eq(GameState.get_material_quantity(EquipmentEnhancer.LEGEND_ORE_ID), 2)
...
func test_elite_bonus_material_actually_added_when_forced() -> void:
	...
	for _i in range(80):
		...
		if not str(bonus.get("material_id", "")).is_empty():
			assert_eq(str(bonus["material_id"]), EquipmentEnhancer.RARE_ORE_ID)
			assert_eq(GameState.get_material_quantity(EquipmentEnhancer.RARE_ORE_ID), 1)
			granted = true
			break
