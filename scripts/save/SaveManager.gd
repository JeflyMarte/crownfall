...
## P3-UX-CRYSTAL-EXCAVATE-001: 発掘ダメージ履歴（ランキング）。
func _migrate_save_v16_to_v17(data: Dictionary) -> Dictionary:
	if not data.has("crystal_excavate_history") or not (data["crystal_excavate_history"] is Array):
		data["crystal_excavate_history"] = []
	return data
...
func _apply_save_data(data: Dictionary) -> void:
	...
	if data.has("crystal_excavate_history") and data["crystal_excavate_history"] is Array:
		GameState.crystal_excavate_history = (data["crystal_excavate_history"] as Array).duplicate(true)
	else:
...
const _VALID_JOB_IDS: PackedStringArray = [
	"swordsman", "ranger", "alchemist", "vanguard", "beast_tamer", "engineer",
]
...
func _apply_roster_save(data: Dictionary) -> void:
	...
	GameState.strip_incompatible_equipped_weapons()
...
func _sync_gacha_roster_metadata() -> void:
	for adv in GameState.roster:
		...
		GachaRarityConfig.apply_stats_for_adventurer(adv)
...
func _apply_gacha_save(data: Dictionary) -> void:
	...
	if data.has("redeemed_codes") and data["redeemed_codes"] is Dictionary:
		var rc: Dictionary = {}
		for k in data["redeemed_codes"]:
			if bool(data["redeemed_codes"][k]):
				rc[str(k)] = true
		GameState.redeemed_codes = rc
	else:
		GameState.redeemed_codes = {}
