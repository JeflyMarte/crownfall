...
const _CONDITION_HINTS: Dictionary = {
	...
	"enemy_has_bleed": "生存敵のいずれかに出血がある。",
	"enemy_has_poison": "生存敵のいずれかに毒がある。",
	"enemy_has_mark": "生存敵のいずれかに標的（mark）がある。",
	"enemy_has_stun": "生存敵のいずれかがスタン状態。",
	"enemy_has_vulnerable": "生存敵のいずれかが脆弱状態。",
	"enemy_has_armor_break": "生存敵のいずれかが防御DOWN状態。",
	"enemy_has_fear": "生存敵のいずれかが恐怖状態。",
	"ultimate_ready": "必殺技のチャージが満タンで発動可能。",
