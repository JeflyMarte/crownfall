...
| UIアイコン | **64×64** | 武器/防具/素材/通貨（現状維持）。★レジェンド武器の作画指示は `docs/art/LegendaryWeaponIcon_Brief.md` |
...
メイン5職と同形式の個別フレーム zip（Idle/Walk/Attack/Hurt/Death）。

| 項目 | 規格 |
|---|---|
| 取込 | `tools/import_gacha_helper_sprites.py`（`~/Downloads/{名前}.zip`） |
| 配置 | `assets/characters/{helper_id}/` |
| SpriteFrames | `resources/animation/CHR_Helper_{suffix}.tres`（`idle`＝walk） |
| 差し込み | `GachaHelperData.sprite_resource_path`。ダンジョンは助っ人優先、UI Idle も `helper_id` フォルダ優先 |
