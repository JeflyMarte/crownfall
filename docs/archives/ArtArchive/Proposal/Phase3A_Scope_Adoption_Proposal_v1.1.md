...
**Status:** Superseded → `../Completed/Phase3A_Scope_Adoption_Completed_v1.1.md`（P3-D001 承認済）
**Type:** Scope Adoption
...
| `ICO_MAT_RelicShard.png` | 素材アイコン | **要確認※1** | Pixel Apprentice | P0 Batch |
...
| `ICO_MAT_EliteRelicShard.png` | 素材アイコン | **要確認※1** | Pixel Apprentice |
| `ICO_MAT_AncientBone.png` | 素材アイコン | **要確認※1** | Pixel Apprentice |
...
| `ICO_MAT_CursedIron.png` | 素材アイコン | **要確認※1** | Pixel Apprentice |
...
## 6. HQ 確認事項（要 Decision）

| # | 論点 | 選択肢 / 現状 |
|---|---|---|
| **D-PA-001** | 素材アイコンのサイズ統一 | v1.0 §3-3 は全アイコン 64×64 を基準とするが、Request Pack v1.0 は素材を 32×32 で指定。ゲーム内表示（小さい）から 32×32 でよいか？ → どちらかに統一が必要 |
| **D-PA-002** | ファイル名サイズサフィックスの要否 | v1.0 は `ICO_WPN_IronSword.png`（サフィックスなし）、Request Pack v1.0 は `ICO_WPN_IronSword_64.png`（`_64` あり）。Godot インポート設定の混乱を避けるため統一要 |
| **D-PA-003** | ENM_Skeleton vs ENM_BoneWalker | Request Pack v1.0 は `ENM_Skeleton_Sheet` で発注、v1.0 は `ENM_BoneWalker_Sheet`。ゲームデータ id は `bone_walker`。ファイル名を `ENM_BoneWalker_Sheet` に統一するのが自然 |
| **D-PA-004** | Bitmap Font の導入判断 | production_theme.tres で等幅ビットマップフォントを導入するか。現行はデフォルトフォント。Pixel Apprentice への外部フォント調達依頼が必要 |
| **D-PA-005** | AutoTile vs 手動マッピング | Godot 4 TileSet の AutoTile（Physics + Terrain）を使うか手動マッピングか。Pixel Apprentice の出力形式（BitMask 付属要否）に影響 |
| **D-PA-006** | 制作バッチの分割方針 | P0 Batch を 1 回で全点依頼か、UI Frame → Icons → Tileset の 3 段階分割か。制作速度と品質レビューサイクルのトレードオフ |
...
| 版 | 日付 | 内容 |
|---|---|---|
| v1.1 | 2026-06-24 | v1.0 を元に Scope Adoption 文書として整備。IN/OUT・Exit Criteria・要求リスト統合・命名矛盾の指摘 |
