...
<<<<<<< HEAD
2026-07-28（**P3-EVT-NONOKA-MEMO-001 GO/Impl**: ギルド情報誌ノノカメモを種別ごと文面プール化。特記なし16本＋他種別複数。スロット決定的抽選）
=======
2026-07-28（**スキル説明ユーザ向け改稿**: 職スキル文面の開発メモ撤去／一覧のCD・tick表記を再使用・平易語へ。パッシブの英語ID・Threatも整理）
>>>>>>> origin/cursor/skill-desc-userfacing-422e
