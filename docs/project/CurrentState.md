...
2026-08-02（**BT／RG支援回復／統合＋main**: Lv50＝獣医の手当て（人12%/オトモ18%）・野営の一滴（自己12%）。要実機）
...
2026-07-31（**戦闘監査案G／統合＋main**: 詠唱off-by-one・パッシブ撃破打ち切り・斉射/余波演出・オトモ発火。要実機）
2026-07-31（**戦闘監査案F／統合＋main**: 敵スキルCD個体別・死者状態・敵クリVFX・オトモレジェンド。要実機）
...
2026-07-31（**戦闘監査案D／統合＋main**: 敵詠唱スロット固定・オトモpartyループ・列AoE確認。要実機）
...
2026-07-30（**攻略Wiki ビルド例**: ゴール編成6種（深層／時環／アンティーク／オトモ／印／Hard）。要公開確認）
...
2026-07-30（**P3-BAL-HELPER-PASSIVE-B001 GO／統合＋main**: ネリオトモ＜ミレイ／シアン小鼓舞必殺接続／レノール平％抑制。要実機）
...
2026-07-30（**ビルド点検案A〜D GO／統合＋main #180〜#182**: パッシブ核寄せ／属性L個性化／レリックDoT・オトモ核／開幕装飾役割分離。要実機）
2026-07-30（**P3-BAL-RELIC-ACC-CD-001**: 案Cレリック（斥候=DoT／軍旗=オトモ）＋案D開幕装飾（王家=初撃／盟約=必殺ゲージ）。要実機）
