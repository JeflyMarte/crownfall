...
2026-07-28（**演出総点検 GO**: 宝箱コメント巻き戻り／非戦闘VFX位置／誤SE修正。`14_Presentation_Pitfalls` 再発防止。統合＋`main`＋Xcode）
2026-07-28（**P3-UI-RARITY-NREL-001 GO/Impl**: 装備レア表示を N＜R＜E＜L（＋M／セット）に統一。キャラ★は据置。統合＋`main`・要実機）
2026-07-28（**P3-BAL-COMBAT-AUDIT-001 GO/Impl 案A**: スナイプ甲砕のみ／威嚇taunt／相棒鼓舞強化／重呪／敵全体Threat按分／断罪0.6／墓鐘前列+stun15%。要実機）
