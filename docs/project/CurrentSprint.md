...
| 助っ人ガチャ | 現状どおり OMIT 維持可（再有効化は β後でも可） |
| BGM | 後回し（Suno・オーナー） |
...
| 設定／クレジット | 音量確認・権利表記（Kenney）画面 | iOS 見据え |
...
| P3-ENEMY-002 残り | 新雑魚（②+4済以外） |
| P3-SUB 表示 | broken_marsh 等寄り道の解禁 |
| P3-CHR / Gacha 再有効 | 助っ人 `GACHA_HELPERS_PLAYABLE` |
| BGM | Suno 配置＋`play_bgm` 配線 |
| P3-UI-BTN-005 | 装備・編成ボタン画像化（002〜004撤回後の再検討） |

### D. 凍結（Decision まで触らない）

天候本格 / 週間日課 / 10連ガチャ / 6装備枠 / Affix本格 / 位置AI本格 / 探索手動+CD / ボタン全面画像化の再開
...
| 2 | 実機通し + QA | ①クリア体験の GO/NO-GO | **オーナー・次** |
| 3 | P3-AUDIO-SE-002 | SE 未配線（任意・推奨） | ✅ Impl（要実機） |
