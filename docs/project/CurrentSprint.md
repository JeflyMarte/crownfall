...
**直近:** 分岐型王痕 **`146` Implemented**（Combat／Save v19／UI／GUT90）。極限制約再設計 `145` Implemented。
...
| 寄り道 | **オミット継続**（`SUB_DUNGEONS_PLAYABLE=false`） |
| 征討 | **イベント常設で配信**（地図なき主／星炉の寝主。⑤クリア後） |
...
| 3b | **P3-BUG-EVENT-RETRY-001** | 曜日イベント：結果「再挑戦」が日次回数を消費しない → **解消**（再挑戦も can/consume。降臨無制限） | ✅ Impl |
...
| ID | 内容 | 状態 |
|---|---|---|
| **P3-DG-EXTREME-001** | **極限任務** EX-01〜10 — 共通基盤＋特殊条件 modifier＋★指令 Save。Decision `143`／**制約再設計 `145`**／**日替わり1任務** | ✅ **Completed**（Phase1+2／制約再設計／日替わり 2026-09-20） |

#### C-5. 王痕分岐（完了）

| ID | 内容 | 状態 |
|---|---|---|
| **P3-DG-ROYAL-MARK-002** | **分岐型王痕** — IIIで攻勢／守勢／技巧。VでUlt覚醒＋最低保証（レノール／火鷹）。`unselected` migration。Decision **`146`** | ✅ **Implemented** |
...
| P3-SUB 表示 | broken_marsh 等寄り道の解禁（征討再定義とは別。寄り道オミット維持） |
| **P3-DG-APEX-NR-BG-001** | 天望専用戦闘BG Early/Late/Boss（開けたステージ・`141`） | ✅ Impl（要実機） |
| **P3-DG-APEX-REDEFINE-001** | 征討再定義 — アストラ・スパイア＋アストラ・カルデラ。Boss 帯 `132`/`135`。セット＝名拒み＋**星炉の滓（`136`）**。星炉BG＋DG ICO／BAN 専用済 | アップデート枠 |
| **P3-DG-APEX-SET-001** | 征討エンシェント **名拒みの冠** — GO＋接続済 | アップデート枠 |
| **P3-DG-APEX-FORGE-SET-001** | 征討エンシェント **星炉の滓** — GO＋接続済（炎上持続／炎上中与ダメ） | アップデート枠 |
| **P3-EQ-ANCIENT-POWER-D-001** | エンシェント地力＋単品パッシブ（案D）— GO＋接続 | アップデート枠 |
| **P3-DG-ABYSS-001** | Biome深層×5。**A/B Impl**／Cレジェンド／D磨きは後続 | アップデート枠 |
| P3-CHR / Gacha 再有効 | ✅ ENABLE-001 済。残り＝専用ドット |
| **P3-GACHA-EQ-KAIWAN-001** | 装備ガチャ限定＝**灰冠の九**。武防飾27＋限定ランダムステ GO済。**封蔵UI／確率案A（Epic55・L内灰冠60）Impl済**／専用mod・パッシブは後続 | アップデート枠 |
| **P3-PET-OTOMO-001** | ペット「ジャック」— NG開始から随伴（4人枠外）・★1・常時前衛 | ✅ Impl（ドット後差し） |
...
| **P3-JOB-ENGINEER-001** | 第6職 **機巧士** — 助っ人3＋パッシブ＋必殺＋**L Phase A**＋**スキルICO**＋**BAL-001（スパイク0.65／甲砕中罠×1.15）** 接続済。残＝専用 CHR／立ち絵／L Phase B（灰冠）。β即投入なし | アップデート枠 |
| **P3-EQ-ENGINEER-LEG-001** | 機巧士ビルドL Phase A — **5点接続済**（罠2／装炎2／破砕装飾1）。Phase B＝双剣梯子・灰冠・専用ICO | ✅ Impl（要実機） |
| **P3-EQ-WARHAMMER-001** | **戦鎚**（`hammer`）— 梯子A接続済（章R0–R2＋破砕L）。専用ICO・灰冠／深層／追加テーマLは後続 | アップデート枠 |
| **P3-UX-CRYSTAL-EXCAVATE-001** | 魔晶石発掘（案A）— 日次1・上限300・日課隣・必殺なし | ✅ Impl（要実機）
| **P3-BAL-CHAR-ULTIMATE-001** | キャラ別必殺（`138`）— テーマ整合・漢字5＋カタカナ。機巧士含む接続済 | ✅ Impl（要実機） |
| （メール要望）エンドコンテンツ | 転生／永続強化／レア周回 — 既存＝深層・降臨・許可点＋**征討再定義（128）**。転生は未Decision |
| （メール要望）発熱・バッテリー | 軽量30fps＋通常45fps・天候ループ抑止 実装済。**パッケージA**（追加ループ抑止・正規化キャッシュ・ダメ数字プール）接続済。確認: `PerfHeat_iOS_Checklist.md` |
...
| — | **P3-BAL-ENEMY-TRICKY-005〜007** | 敵トリッキー第2波（吸血／加速／沈黙／召集） | ✅ Impl（要実機） |
...
| — | **P3-BAL-ULTIMATE-AOE-001** | タイタンロア全体＋ドミニオン多重デバフ | ✅ Impl（要実機） |
| — | **P3-BAL-ULTIMATE-ROLE-001** | VG全体バフ／SW全体攻撃／AL全体回復 | ✅ Impl（要実機） |
| — | **P3-BAL-HEAL-MAXHP-001** | 味方回復を対象maxHP割合へ | ✅ Impl（要実機） |
...
| — | **P3-UX-WIPE-CAUSE-002** | 全滅敗因詳細化（2〜3行・群れ／ヒント最大2） | ✅ GO・Impl |
...
| — | **P3-SKILL-VG-TRIAD-001** | VG＝味方バフ／ヘイト／攻撃の三軸再編 | ✅ GO・統合＋main |
...
| — | **P3-UX-BATCH-20260727** | レリック紫枠／ボス・エリートテロップ二重解消／装備一覧軽量化／状態異常リンク修復 | ✅ GO・統合＋main＋Xcode |
