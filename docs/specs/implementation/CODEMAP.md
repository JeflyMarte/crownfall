...
| RoyalMarkScene | `scenes/royal_mark/RoyalMarkScene.tscn` | `scripts/royal_mark/RoyalMarkScene.gd`（**王痕育成専用UI `144` Phase2**・次王痕主情報／Reward→Cost→CTA・現在効果1行・III/Vコンパクト・キャラ管理導線。**方針分岐 UI は `146` Approved・未実装**） |
...
| `royal_mark/` | `RoyalMarkScene.gd`／`RoyalMarkUiTokens.gd`（**王痕育成専用UI `144` Phase2**・次王痕主情報階層・Skill/Ultimate 強化行。**`146` 方針分岐は未実装**） |
