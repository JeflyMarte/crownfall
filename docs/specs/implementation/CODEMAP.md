...
| ダンジョン | `resources/dungeons/mourngate.tres`, `whisperwood.tres`, `mistfen.tres` + 寄り道 `broken_marsh.tres`（ドロップ・プールは `DungeonData.weapon/armor/accessory_pool`・P3-D154。解放条件は `unlock_after_dungeon_id`/メイン直列・P3-D157） |
