...
| RoyalMarkScene | `scenes/royal_mark/RoyalMarkScene.tscn` | `scripts/royal_mark/RoyalMarkScene.gd`（**王痕育成専用UI `144` Phase2**・拠点左メニュー導線） |
...
| `royal_mark/` | `RoyalMarkScene.gd`（**王痕育成専用UI `144` Phase2**） |
...
| `systems/` | **`LevelSystem.gd`**（**Lv99上限** P3-LV-099・Lv51+逓減成長・**P3-BAL-GROWTH-H1-001** HP/ATK/DEF＋キャラ別倍率）・**`RoyalMarkConfig.gd`／`RoyalMarkSystem.gd`**（王痕 I〜V・`144`）・**`SkillProgression.gd`**・**`WeaponSkillHelper.gd`**・`JobEvolution.gd`／**`EvolutionTraits.gd`**／`EvolutionVisual.gd`（到達形・βは `JOB_EVOLUTION_PLAYABLE=false`／P3-JOB-EVO-OMIT-001） |
