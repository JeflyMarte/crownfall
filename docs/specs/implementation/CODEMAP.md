...
**危険度ティア（P3-DG-TIER / P3-BAL-NM-CAP99-001）:** `DungeonTierConfig.gd` — Hard/NM はメイン5キャンペーン周回帯。解放=ノーマル全クリア／ハード全クリア。敵／推奨Lvボーナス= `75−n_end` / `99−n_end`（終端推奨≈75／99）。UI=`DungeonSelectScene` TabsRow。**降臨／征討**（時王・境界廊・アストラ・スパイア）はキャンペーン条件なしで N/H/NM 自由選択（`P3-UX-DESCENT-TIER-TABS-001`／`P3-DG-APEX-TIER-001`）。征討雑魚Lvは階帯表（`ApexConquestConfig`／`133`）。  
...
| EquipmentScene | `scenes/equipment/EquipmentScene.tscn` | `scripts/equipment/EquipmentScene.gd`（**所持 B/C**: 切替は差分パッチのみ／`EquipmentInventoryIndex` 事前計算／**P3-UX-EQUIP-SCROLL-PERF-001** 仮想スクロール／**王痕 Rank 参照のみ `144` Phase2**） |
| RoyalMarkScene | `scenes/royal_mark/RoyalMarkScene.tscn` | `scripts/royal_mark/RoyalMarkScene.gd`（**王痕育成専用UI `144` Phase2＋`146` 方針分岐**・次王痕主情報／Reward→Cost→CTA・現在効果1行・III/Vコンパクト・方針パネル・excluded警告・キャラ管理導線） |
...
| CrystalExcavateSelectScene / Combat / Result / Ranking | `scenes/excavate/*.tscn` | `scripts/excavate/CrystalExcavate*.gd` + **`CrystalExcavateSystem.gd`** + **`CrystalExcavateBgHelper.gd`**（**P3-UX-CRYSTAL-EXCAVATE-001** 日次魔晶石発掘。BG=`UI_BG_CrystalExcavate*.png`／結果バナー／ダメージ履歴ランキング） |
...
| CommanderPermitScene | `scenes/commander/CommanderPermitScene.tscn` | `scripts/commander/CommanderPermitScene.gd`（特権強化フレーム重ね：−／＋ドラフト／決定セーブ） |
...
- 戦闘ロジック配線（`DungeonScene.gd`）— CT/ATB・5 スロット戦術（P3-D084〜086）・Threat/陣形（P3-D104/106）・混成/個別ターゲット（P3-D110/111）・詠唱（P3-D112）・スキルローテ（P3-D113）・遺物トリガ（P3-D114）・連携（P3-D115）・ボスフェーズ（P3-D116）・探索スキル（P3-D117）・戦闘スキップ（P3-D118）・**必殺＝時間チャージ＋下ゲージ2段（P3-COMBAT-GAUGE-001／P3-BAL-ULTIMATE-TIME-001）**・**スキルCD＝戦闘クロック（P3-BAL-SKILL-CD-TIME-001／`55_SkillCdRealtime`）**・**必殺カットイン（P3-UX-ULTIMATE-002）**
...
- `DungeonSelectScene` — ルートタブ（メイン／イベント／無限）。**DungeonRouteGuideOverlay**（P3-DG-ROUTE-GUIDE-001: イベント／降臨／征討／無限の手引きボタン＋初回P分け）
...
| `combat/` | **コア:** `CombatController.gd`（`class_name`・CT/ATB・Threat・群れ/混成・個別ターゲット・詠唱・ボスフェーズ index・**怒涛累積CT** P3-BAL-COMBAT-ATTRITION-001）, `SkillExecutor.gd`, `StatusResolver.gd`, `StatusInstance.gd`, `ElementResolver.gd`, **`DamageCalculator.gd`**（ダメージ式 SSOT・シーン非依存 static・P3-REF-001）, **`BalanceConfig.gd`**（グローバルバランス定数 SSOT・P3-BAL-005・**ATTRITION_***）, **`EngineerTraps.gd`**（機巧士仕掛け印・P3-JOB-ENGINEER-001） |
| | **戦術/AI:** `CombatTactics.gd`（行動方針6・重み付きルーレット・回復閾値／バフ再付与／スキル内カテゴリ偏り・旧IDマップ）, `CombatGambit.gd`（条件ラベル等・カスタム実行はオミット）, **`BossSummonLayout.gd`**（ボス呼び出し連れの左右手前配置） |
...
| `dungeon/` | `DungeonController.gd`, `DungeonScene.gd`（生態素材ドロップ・図鑑方針ボーナス P3-D128・**曜日イベント撃破EXP/Gold×2 P3-BAL-WEEKDAY-EVENT-REWARD-001**・**分かれ道三択 P3-DG-FLOOR-CHOICE-001**）・**`FloorChoiceOverlay.gd`**・**`DungeonTierConfig.gd`**（危険度ティア P3-D164）・**`BiomeBuildThemes.gd`**（章別ビルド傾向＋象徴雑魚 id P3-BAL-BIOME-BUILD-THEME-001）・**`AbyssDungeonConfig.gd`**（深層階数帯／**敵Lv絶対表 P3-DG-ABYSS-LV-001**）・**`AbyssMilestoneRewards.gd`**／**`AbyssLegendaryWeapons.gd`**／**`AbyssLegendaryDrop.gd`**（深層マイルストーン・レジェンド付与・低確率天井）・**`EventExclusiveRewards.gd`**（降臨セット装備ドロップ P3-DG-EVENT-SET-001）・**`EventDungeonSchedule.gd`**（曜日開放＋時間帯降臨 chronos 0/3/6/9・valgard 1/4/7/10・`is_weekday_event`・極限は常設 `ExtremeMissionConfig.is_open_now`）・**`ExtremeMissionConfig.gd`**（極限任務 EX-01〜10・共通 modifier／`143` Completed・**制約再設計 `145`**・**常設出現**・**王痕片付与 `144`**）・**`EnemyTierVariantConfig.gd`**（Hard/NM 呼称・個性 P3-ENEMY-TIER-VAR）・**`WanderingEnemyConfig.gd`**（遍在希少種 P3-D166）・**`ShadowStalkerLoot.gd`**（影狩死告武器 P3-EQ-SHADOW-DEATHREAP-001） |
| `equipment/` | **所持インデックス B/C**（`EquipmentInventoryIndex`／切替で袋非再構築）／**状態付与一本化**（`EquipmentRandomMods`／P3-EQ-STATUS-UNIFY-001: 専用炎上付与等→`on_hit_status`）／**平坦帯圧縮**（`51_FlatRollNarrow.md`／上限×0.7・下限55%）／**焼直し**（`EquipmentReforgeHelper`／P3-FORGE-REFORGE-001）／**装備総合力**（`EquipmentPower`／P3-EQ-POWER-RECOMMEND-002・主ステ寄り・非表示・おすすめ比較） | `EquipmentController.gd`, `EquipmentScene.gd`（**王痕 Rank 参照のみ `144` Phase2**）, **`EquipmentInventoryIndex.gd`**, **`EquipmentUiHelper.gd`**（P3-UI2-019）, **`EquipmentUiTokens.gd`**（装備 chrome）, **`BuildTagHelper.gd`**（P3-UI2-016）, **`EquipmentRandomMods.gd`**（P3-EQ-DIABLO-001）, **`EquipmentReforgeHelper.gd`**, **`EquipmentPower.gd`**, **`EquipmentRecommendHelper.gd`**, **`AffixRoller.gd`**（レガシー）, **`AffixStatCalculator.gd`**, **`AffixDisplayFormatter.gd`**, **`JobStatCalculator.gd`** |
| `royal_mark/` | `RoyalMarkScene.gd`／`RoyalMarkUiTokens.gd`（**王痕育成専用UI `144` Phase2＋`146` 方針**・次王痕主情報階層・Skill/Ultimate／方針変更） |
...
| `result/` | `ResultScene.gd`（**P3-UX-WIPE-CAUSE-002** 全滅敗因2〜3行・素材アイコン P3-D135・**生産レシピ解放＝今回のみ** P3-UX-RESULT-CRAFT-UNLOCK-001・**P3-UI2-023** パネル/フッター polish・**P3-UX-SKILL-LEARN-PERSIST-001** 習得を名横に常時表示・**P3-BAL-CLEAR-EXP-001** クリアボーナス行・**極限王痕片行 `144`**） |
| `save/` | `SaveManager.gd`（**SAVE_VERSION 20・王痕 `144`＋方針 paths `146`・片表示×10**） |
| `systems/` | **`LevelSystem.gd`**（**Lv99上限** P3-LV-099・Lv51+逓減成長・**P3-BAL-GROWTH-H1-001** HP/ATK/DEF＋キャラ別倍率）・**`RoyalMarkConfig.gd`／`RoyalMarkSystem.gd`／`RoyalMarkSkillModifier.gd`**（王痕 I〜V・方針 offense/defense/technique・排他 Job／Ult・`144`＋`146`）・**`SkillProgression.gd`**・**`WeaponSkillHelper.gd`**・`JobEvolution.gd`／**`EvolutionTraits.gd`**／`EvolutionVisual.gd`（到達形・βは `JOB_EVOLUTION_PLAYABLE=false`／P3-JOB-EVO-OMIT-001） |
...
| 敵 | `resources/enemies/`（`EnemyData.basic_attack_name`＝通常攻撃頭上名・**P3-UX-ENEMY-BASIC-NAME-001**）— メイン（①〜⑤・P3-ENEMY-002 拡充）+ 征討／降臨 Boss（chronos_wave→`chronos_mausoleum` / valgard→`valgard_boundary` / skarpedion / mycolga_ancient / karna_smoke / nereion_depths / **forgedormient＝征討アストラ・カルデラ・降臨帯 `135`** / **albark＝征討アストラ・スパイア・降臨帯 `132`**）+ 遍在希少種4（P3-WANDER-002/004: cosmic_duck / crown_raven / golden_scarab / shadow_stalker）+ **ビッグコズミックダック**（裂け目ボス＋Hard+放浪昇格／裂け目COMBAT 1〜2・P3-ENEMY-BIG-COSMIC-DUCK-001/002）+ ロックバイソン（P3-BAL-ROCK-BISON-WANDER-001: 放浪1.5%・岩角／境界廊のみ pool・素材率1.75） |
| ダンジョン | `resources/dungeons/` — **38本**: メイン5 + 寄り道5 + 征討8 + イベント5 + **深層5**（`abyss_*`／**P3-DG-ABYSS-001**）+ **極限10**（`ex_*`／**P3-DG-EXTREME-001** Completed）。`route_type`: main/side/apex/event/abyss/extreme。イベントは日次挑戦枠（DGごと）。**章データがある Biome は route 不問でバナー下にサブ章**（P3-DG-EVENT-STG-001）。イベントは各1章。深層は無限階・親Biomeクリアで解放。深層限定レジェンド武器5（`abyss_veinblade` 等／**P3-DG-ABYSS-001-C**）。**征討配信中**: `north_reach`（アストラ・スパイア）／`red_forge_depths`（アストラ・カルデラ・`134`）。**極限配信中**: EX-01〜10（**極限任務タブ**常設・Decision `143`） |
...
| 日課 | `resources/daily_missions/` — プール抽選3件/日（P3-DAILY-002）。報酬最大2種・ランダム装備N〜E可（P3-BAL-DAILY-REWARD-VARIETY-001）。**招待日課はプール外**（P3-BAL-GACHA-001） |
