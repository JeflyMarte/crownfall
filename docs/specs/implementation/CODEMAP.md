...
| | **パッシブ/シナジー:** `CombatPassives.gd`, **`AbyssWeaponEffects.gd`**（深層レジェンド戦闘状態 P3-DG-ABYSS-001-C）, `CombatSynergy.gd`, `CombatTags.gd`, `CombatCombos.gd`（P3-D109）, **`EnemyResistTelop.gd`**（T6/T7 初回軽減テロップ）, 敵トリッキー第2波=`haste`/`silence`/`summon`＋`lifesteal_ratio`（`36_EnemyTrickyWave2.md`）, **群れ人数連動**=`BalanceConfig.swarm_density_*`（`37_SwarmDensitySoloPressure.md`） |
