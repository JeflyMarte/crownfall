...
| `dungeon/` | `DungeonController.gd`, `DungeonScene.gd`（生態素材ドロップ・図鑑方針ボーナス P3-D128・**曜日イベント撃破EXP/Gold×2 P3-BAL-WEEKDAY-EVENT-REWARD-001**・**分かれ道三択 P3-DG-FLOOR-CHOICE-001**）・**`FloorChoiceOverlay.gd`**・**`DungeonTierConfig.gd`**（危険度ティア P3-D164）・**`BiomeBuildThemes.gd`**（章別ビルド傾向 P3-BAL-BIOME-BUILD-THEME-001）・**`AbyssDungeonConfig.gd`**（深層階数帯／**敵Lv絶対表 P3-DG-ABYSS-LV-001**）・**`AbyssMilestoneRewards.gd`**／**`AbyssLegendaryWeapons.gd`**／**`AbyssLegendaryDrop.gd`**（深層マイルストーン・レジェンド付与・低確率天井）・**`EventExclusiveRewards.gd`**（降臨セット装備ドロップ P3-DG-EVENT-SET-001）・**`EventDungeonSchedule.gd`**（曜日開放＋時間帯降臨 chronos 0/3/6/9・valgard 1/4/7/10・`is_weekday_event`）・**`EnemyTierVariantConfig.gd`**（Hard/NM 呼称・個性 P3-ENEMY-TIER-VAR）・**`WanderingEnemyConfig.gd`**（遍在希少種 P3-D166） |
