...
| 敵待機が拡大縮小パルス（シート内の体高さ揺れ） | PixelLab が足元固定で体を縮める。**`--freeze` は不自然**。import 未更新でも旧ズーム残存 | `stabilize_enemy_sheet_scale.py --max-dh` で安定セル循環（オルド≈2／ストーム≈10）。差替後 `--import`。Hard/Nightmare 同処理 |
