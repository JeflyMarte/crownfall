...
| 敵待機が拡大縮小パルス（シート内の体高さ揺れ） | PixelLab が足元固定で体を縮める。アップスケール焼き直しは横に太る。**PNG差替後も `.godot/imported` が古いと旧ズームが残る** | `tools/stabilize_enemy_sheet_scale.py`（重度は `--freeze`）。差替後 `godot --headless --import`。Hard/Nightmare も同処理 |
