...
**Version:** v1.5  
**Approved:** 2026-06-23（§7.1 追記: 2026-07-17／Cursor 一本化: 2026-07-22／**修正＝統合セット既定: 2026-07-30**）  
**Decisions:** P2-D154〜P2-D156, **P2-D169**（設計フロー v1.1）, **P3-OPS-CURSOR-001**, **P3-OPS-FIX-MERGE-SET-001**  
...
4. 合格（**§7.1.1 の既定**／またはオーナー明示 GO）→ **§7.1 のとおりブランチへ反映** → ダッシュボード更新  
...
| **Impl／修正の完了（§7.1.1）** | **必ず** 現行統合ブランチへマージ（＋ push）。**同じタイミングで `main` にも上げる** |
...
#### 7.1.1 既定: 修正 ＝ コミット＋統合＋`main`（セット）

**オーナー方針（2026-07-30 — P3-OPS-FIX-MERGE-SET-001）:**  
オーナーが **特に止めない限り**、実装・修正の完了は次を **同じターンでセット実行**する。別途「GO」待ちにしない。

| 手順 | 内容 |
|---|---|
| 1 | 実装／修正 |
| 2 | コミット（＋必要なら feature ブランチ push） |
| 3 | 現行統合ブランチへマージ → push |
| 4 | **同じタイミングで `main` へ** → push |
| 5 | `CurrentState.md`（該当時 Decision Log／CODEMAP）を更新 |

**セットにしない場合（オーナーが明示したときのみ）:**

- 「レビューだけ」「マージしない」「WIP」「PR だけ」「Decision のみ」
- STOP（体験・面白さ・システム懸念／曖昧指示）→ 実装せず確認
- 壊れている／未検証で `main` に載せられないと判断したとき（統合仮置き可・`main` 禁止）
...
**Closeout 確認（Impl／修正完了時に HQ が見る）:**
...
#### 7.1.2 セットでもリポジトリ外／後続になりやすいもの（漏れ注意）

コミット＋統合＋`main` だけでは **自動では揃わない**／**別確認が要る**もの:

| 種類 | 例 | 扱い |
|---|---|---|
| ダッシュボード | `CurrentState`／`CurrentSprint` | **セット内で更新**（漏らし禁止） |
| 仕様記録 | Decision Log／CODEMAP／brief | 設計・API・アート方針が変わったら同ターン |
| 再発防止 | `known-pitfalls.mdc`／領域 `.mdc` | 起動・パース・同種再発時は同ターン |
| 検証 | GUT／smoke／headless | 変更種別に応じて同ターン実行。実機はオーナー |
| 実機・ビルド | iOS 再インストール／Godot 再インポート | リポジトリ外。応答の「残り」に書く |
| PR クローズ | GitHub PR の closed | 本線反映後に閉じる（残っていても本線が正） |
| 未依頼の横展開 | 同種バグの他画面 | スコープ外なら「残り」に候補のみ |
...
| v1.5 | 2026-07-30 | §7.1.1 修正＝コミット＋統合＋`main` セット既定（P3-OPS-FIX-MERGE-SET-001）。§7.1.2 漏れ注意 |
