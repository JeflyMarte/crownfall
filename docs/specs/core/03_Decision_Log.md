...
| P3-D036b-2 | **通貨 `gacha_token`（無償のみ）**。入手・単価の現行正は **P3-BAL-GACHA-001**（単発500・クリア35–65等）。本行の「ラン成功1〜2／100G購入」は **廃止** | 初期決定。後続で上書き |
| P3-D036b-3 | **引き方=単発のみ**。10連なし（UI削除済 P3-GACHA-UI-TRIM-001）。単価は **500 魔晶石**（P3-BAL-GACHA-001） | プール小のため |
...
| P3-ECO-002-3 | **ロジック不変** — ~~単発1・100G購入・ラン成功1〜2~~ → **現行は P3-BAL-GACHA-001**（単発500・購入なし・クリア35–65・天井廃止） | 表示統一時は据置。経済は後続で上書き |
...
## 調査室バランス点検（2026-07-25 — P3-BAL-SURVEY-001）

> **オーナー GO（A＋C）** — 調査室報酬を潜行より一段下に。図鑑段階の SURVEY 加算を配線。

| # | 決定 | 根拠 |
|---|---|---|
| P3-BAL-SURVEY-001-A | **魔晶石** — 短 **15–30**／標準 **40–70**（旧 20–40／80–140）。潜行クリア 35–65 より一段下 | Decision「潜行より安定・一段下」 |
| P3-BAL-SURVEY-001-A2 | **日次 SURVEY 上限後** — 受取魔晶石（と連動 Gold）**半減**。素材・武器は据置 | 放置石稼ぎ抑制 |
| P3-BAL-SURVEY-001-C | **図鑑段階** — 段階アップごとに① SURVEY **+2%**（差分段階×）を `GameState` から配線 | 死コード解消・Decision §10 どおり |
| P3-BAL-SURVEY-001-SSOT | **SSOT** — `decisions/05_HubSurveyRoom.md` §10 を本 Decision で更新 | 数値の正 |
...
### 完全調査景品テーブル（2026-07-26 — P3-SURVEY-COMPLETE-001）

> **オーナー GO** — 案A'′。②④はペット＋資源。★4は低確率。★2券新設。ガチャチケットを階段に含める。

| # | 決定 | 根拠 |
|---|---|---|
| P3-SURVEY-COMPLETE-001-1 | メイン①〜⑤の SURVEY **100%** で **一回限り** 景品付与（`hub_survey_complete_claimed`） | 完走ご褒美 |
| P3-SURVEY-COMPLETE-001-2 | **①** 石30＋Gold500＋ガチャ券×1 | 序盤ご褒美 |
| P3-SURVEY-COMPLETE-001-3 | **②** 石50＋Gold800＋素材＋ガチャ券×1 ＋（未所持）アッシュ | ペット後も資源が残る |
| P3-SURVEY-COMPLETE-001-4 | **③** 石80＋Gold1200＋素材＋**LB★2×1**、ガチャ券+1は **50%** | ★2券導入 |
| P3-SURVEY-COMPLETE-001-5 | **④** 石100＋Gold1500＋素材＋**LB★3×1** ＋（未所持）インク、**LB★4 3%**（外れは石+40） | 高難易度＋低確率★4 |
...
## レジェンド武器ドロップ気づき＋レート（2026-07-22 — P3-EQ-LEG-DROP-001）

> オーナー: 落ちても気づかない／レートを下げたい。代わりにボス以外からも出てよい。

| # | 決定 | 根拠 |
|---|---|---|
| P3-EQ-LEG-DROP-001-1 | **入手源** — 武器★は撃破直ドロップのまま（雑魚／エリート／ボス）。ボス限定にしない | オーナー了承のトレードオフ |
| P3-EQ-LEG-DROP-001-2 | **重み** — COMMON/RARE/EPIC を ×3（120/45/15）、LEGENDARY=1据置 → ★相対約1/3 | 供給過多を抑えつつ全敵から出る |
| P3-EQ-LEG-DROP-001-3 | **気づき演出** — SE `legendary_drop`（gacha_reveal）＋宝箱音、金フラッシュ、紙吹雪、テロップ、大きめアイコン | 遺物Getと同系統で見逃し防止 |
| P3-EQ-LEG-DROP-001-4 | **対象** — 武器／防具／装飾の ★以上（神話含む）。ボス初回★防具装飾も同演出 | レア帯の体感を統一 |
| P3-EQ-LEG-DROP-001-5 | **スコープ外** — 防具★のボス初回確定（P3-EQ-LEG-001）は変更しない | 別レーン |
...
## ボス召喚撃破で章★が付く事故（2026-08-09 — P3-FIX-BOSS-LEG-SUMMON-001）

| ID | 決定 | 理由 |
|---|---|---|
| P3-FIX-BOSS-LEG-SUMMON-001-1 | **章★／神話／イベントボス報酬はボス本体撃破時のみ**（`is_run_boss_kill`）。召喚・護衛は対象外 | ネレイオン召喚鮫撃破で潮鳴王の鎧が出た |

## 撃破報酬ゲート（護衛・召喚・二重付与）（2026-08-09 — P3-FIX-KILL-REWARD-GATES-001）

詳細: `docs/specs/decisions/108_KillRewardGates.md`
...
## 敵与ダメ圧力＋回復封じ（2026-08-09 — P3-BAL-ENEMY-PRESSURE-001）

詳細: `docs/specs/decisions/109_EnemyPressureHealBlock.md`

| ID | 決定 | 理由 |
|---|---|---|
| P3-BAL-ENEMY-PRESSURE-001-1 | **`ENEMY_GLOBAL_ATK_MULT` 1.30→1.625**（×1.25） | 2-1以降の殴りが薄い |
...
## 被弾ガード半永久化の抑制（2026-08-09 — P3-BAL-GUARD-ONHIT-UPTIME-001）

詳細: `docs/specs/decisions/110_GuardOnHitUptime.md`
...
## 硬殻敵の圧力スキル（2026-08-09 — P3-BAL-HARD-SHELL-PRESSURE-001）

詳細: `docs/specs/decisions/112_HardShellPressureSkills.md`
...
| P3-EQ-LVL-001-7 | **装着時クリップ禁止（2026-08-02）** — 拾得／錬成の装備Lvは永続値。装着でキャラLvへ下げない。上限は EXP 成長のみ | 高Lv装備が装着で破壊される事故の修正。誤実装 `clamp_equip_level_to_member` 廃止 |
...
| P3-UX-RESULT-004-2 | **MVPスコア数値は画面非表示**。選出ロジックは維持。表示は与ダメ／最大ヒット／回復のみ | オーナー: わかりにくいので出さない |
...
| P3-FORGE-003-5 | **返却（ベース）** — 装備レア＝P3-MAT-004 主鉱石。◇=基礎鉱×**3**+共通×1 / ◆=蒼古×1+共通×1 / ✦=深層×1+共通×2 / ★=王墓×1+共通×2（**P3-BAL-DISMANTLE-001** で◇を2→3） | 案1スクラップ |
...
| P3-STORY-STARTER-001-10 | **2026-08-09** — **5-5（フロストリッジ）は仲間加入なし**。開始1＋①〜④×-5で初期5。5-5はニーナ功績のみ | オーナー指示 |
...
| P3-UI-TITLE-001-5b | **拠点の設定からもタイトルへ** — システム欄に「タイトルへ戻る」（確認＋セーブして Title）。戻る／拠点へは従来どおり | 2026-07-28 オーナー |
...
| P3-FORGE-ALCHEMY-001-5b | **追記 2026-08-07** — **装備中も錬成可**（主材・素材）。素材は実行時に外して消滅。装備中素材は確認ダイアログ対象。神話は従来どおり不可 | オーナー指示 |
...
## 鍛冶バランス点検是正（2026-07-25 — P3-BAL-FORGE-002）

> **オーナー GO（A〜E 全推奨値）** — 生産／炉研ぎ／錬成／分解の点検後是正。

| # | 決定 | 根拠 |
|---|---|---|
| P3-BAL-FORGE-002-1 | **見習いの杖レシピ** — `relic_shard×2` + **`ancient_bone×1`** + **Gold40**（◆コスト整合） | 最安レア武器ルート封じ |
| P3-BAL-FORGE-002-2 | **錬成Gold** — 旧 `ceil(30 × 実上昇 × 帯)` → HEAVY で60 → **現行は P3-BAL-ALCHEMY-GOLD-AB-001**（120×帯1.5/2/3） | 装備Lv加速の主経路を抑える |
...
## キャラEXP／成長カーブ点検（2026-07-25 — P3-BAL-EXP-001）

> **オーナー GO（推奨どおり）** — 卓上点検。数値変更は見送り。仕様同期＋計測ツール修復のみ。
...
| P3-BAL-EXP-001-E | **敵EXP係数 0.15→0.18** — **任意・今回見送り** | 中盤周回が重い実感が出てから |
| P3-BAL-EXP-001-F | **Lv51+ 必要EXP緩和** — **任意・今回見送り** | エンド短縮需要が出てから |
| P3-BAL-EXP-001-G | **`balance_sim` 修復** — `LevelSystem` が `-s` 時に autoload 識別子でパース失敗しないよう実行時解決 | EXP実測ツール復旧 |
...
## 序盤1ダメ／一撃感の再調整（2026-07-22 — P3-BAL-OPENING-002）

> **オーナー GO 案C** — 敵被ダメが約1・Lv1で味方ほぼ一撃。式の非対称が主因。

| # | 決定 | 根拠 |
|---|---|---|
| P3-BAL-OPENING-002-1 | **敵→味方も逓減式** `× K/(K+DEF)`（味方→敵と同式）。旧 flat `ATK−DEF` 廃止 | DEF≧ATKで常時1ダメになる穴を塞ぐ |
| P3-BAL-OPENING-002-2 | **敵グローバル HP ×2.00**（ATK×1.30 据置） | 数撃交換。脅威は式修正で出す |
| P3-BAL-OPENING-002-3 | **味方 ATK ボーナスのみ ×0.40**（HP/DEF は ×0.70 据置） | 一撃感を抑える。耐久は維持 |
| P3-BAL-OPENING-002-4 | **全ダンジョン共通**（`BalanceConfig` / `DamageCalculator`） | OPENING-001 と同じ適用範囲 |
## 群れ出現の強化＋護衛（2026-07-22 — P3-BAL-SWARM-001）
> **オーナー GO 案C** — モーンゲート通しで複数敵が少ない。強い敵は雑魚を従えてよい。
| P3-BAL-SWARM-001-1 | **群れ率** `SWARM_CHANCE` 0.24→**0.45**（安全優先の半減は維持） | 案C・頻度を上げる |
| P3-BAL-SWARM-001-2 | **雑魚の群れ対象拡大** — 墓鐘バット／水晶ハリネズミ／ルーンローチを `can_swarm`。既存ハウンド／ネズミ含め上限 **2〜4**（ローチは2〜3） | 対象＋頭数 |
| P3-BAL-SWARM-001-3 | **護衛リーダー** `EnemyData.escorts_minions` — 骸面マンティス／水晶スコーピオン。群れ時の追加枠は同種複製せず、プール内の雑魚（`can_swarm` かつ非護衛）を従える | 「強い敵＋雑魚」要望 |
| P3-BAL-SWARM-001-4 | **ELITE/BOSS・放浪は従来どおり単体** | 既存ルール維持 |
| P3-BAL-SWARM-001-5 | 混成候補からも護衛リーダーを除外（雑魚同士の混成のみ） | マントスが雑魚群れの「別種」に混ざらない |
---
## 群れの全Biome展開＋ティア別強化（2026-07-22 — P3-BAL-SWARM-002）
> **オーナー GO** — 群れ／護衛を他メイン・サブにも。Hard／Nightmareで率と質を上げる。
| P3-BAL-SWARM-002-1 | **メイン5＋寄り道5**の雑魚に `can_swarm` を横断付与。各メインに護衛リーダー1（WW=ルーンカルキノス／MF=沼地の王／BS=深淵イカ／FR=ストームジョー）。寄り道は親Biome敵を共有 | 体感を全ルートで揃える |
| P3-BAL-SWARM-002-2 | **ティア倍率**（`DungeonTierConfig`）— 群れ率 N×1.0／H×1.25／NM×1.50 | 難易度上昇で頻度↑ |
| P3-BAL-SWARM-002-3 | **質** — サイズボーナス N+0／H+1／NM+2（上限5）＋混成率 N0.50／H0.65／NM0.80 | 頭数と異種混成で質↑ |
...
| P3-EVT-FIELD-001-2 | **UI名＝「今日のダンジョン状態」**（旧「いまの野外」） | オーナー表記変更 |
...
| # | 決定 | 根拠 |
|---|---|---|
| P3-DAILY-002-1 | **表示** — 拠点行は `title` のみ。内容が1行で分かる文言（例「装備品を1つ作成する」） | オーナー指定 |
| P3-DAILY-002-2 | **戦闘勝利廃止** — `kill_enemy`「モンスターを20体倒す」。ボス・エリート撃破も加算 | オーナー B＋含む |
| P3-DAILY-002-3 | **毎日3件をプール抽選** — `day_key` シードの決定的シャッフル。未受取は日跨ぎ失効（従来どおり） | オーナー指定 |
| P3-DAILY-002-4 | **プール** — クリア／撃破20／エリート1／ボス1／生産／強化／錬成／分解／招待1 | 既存導線のみ |
| P3-DAILY-002-5 | **スコープ外** — 週間日課・専用シーン・スタミナ連動 | 据置 |
...
| # | 決定 | 根拠 |
|---|---|---|
| P3-WANDER-004-1 | **追加** — `golden_scarab`（1.5%）／`shadow_stalker`（0.8%・表示名「影狩り」）。既存ダック2.5%／レイヴン1.5%据置（案A） | 役割マトリクス維持・過密抑制 |
| P3-WANDER-004-2 | **スカラベ** — gold 220・EXP低・装備0・**3行動後逃走** | ダックの金版 |
| P3-WANDER-004-3 | **影狩り** — EXP100／gold100／装備75%・レア重み5/15/40/40・神話別枠2%・逃走なし。伝説プール補完はレイヴンと同経路 | 高リスク総取り。レイヴンより装備率は低め |
| P3-WANDER-004-4 | ティア倍率は既存と同型（N×1.0／H×1.3／NM×1.6） | P3-WANDER-003 整合 |
| P3-WANDER-004-5 | **P1のみ** — 放浪差し込み＋図鑑文＋unit。イベントDG／野外目撃増はスコープ外 | β必須ではない |
| P3-WANDER-004-6 | ドットはプレースホルダ（スカラベ＝ローチ／影狩り＝骸面マンティス） | アート後差し |

**Closeout（2026-07-23）:** データ＋Config＋ドロップ配線＋unit。

## 遍在希少種イベントDG追加（2026-07-23 — P3-DG-WANDER-EVENT-002）

> オーナー指示: ゴールデンスカラベ／影狩りにも日次イベントDGを作成。ダック／レイヴンと同型（各1章・日1回・別カウント）。

| # | 決定 | 根拠 |
|---|---|---|
| P3-DG-WANDER-EVENT-002-1 | **`golden_nest`（砂金の巣穴）** — event・5F・Bossなし・日1回・敵=`golden_scarab`のみ・放浪無効・群れ10% | スカラベ＝金寄り短編 |
| P3-DG-WANDER-EVENT-002-2 | **部屋** — COMBAT35 / treasure30 / trap20 / heal10 / lore5 | 宝箱と金ピンチ |
| P3-DG-WANDER-EVENT-002-3 | **`shadow_hunt`（影狩りの狩場）** — event・5F・Bossなし・日1回・敵=`shadow_stalker`のみ・放浪無効・群れ5% | 強敵ハンティング |
| P3-DG-WANDER-EVENT-002-4 | **部屋** — COMBAT55 / treasure15 / trap15 / heal10 / lore5。装備プールはモーンゲート帯流用 | レイヴンより戦闘寄り・高難度 |
| P3-DG-WANDER-EVENT-002-5 | 章は各 `*_1_1` のみ（P3-DG-EVENT-STG-001 共通）。日次枠は既存2種と **別カウント** | 一日最大4イベント |
...
## 特典コード受取（2026-07-23 — P3-UX-REDEEM-001）
> **オーナー指示** — 設定画面にコード入力→特典受取。未指定項目は推奨案で確定。

| # | 決定 | 根拠 |
|---|---|---|
| P3-UX-REDEEM-001-1 | **UI** — 設定に「特典コード」節。入力欄＋「受け取る」。タイトル／拠点の両方から利用可 | オーナー指示 |
| P3-UX-REDEEM-001-2 | **検証** — オフライン固定表（`RedeemCodeCatalog`）。サーバーなし | β向け最小 |
...
## 在野助っ人を人物台帳へ（2026-07-23 — P3-W-GACHA-NAMED-001）
> **オーナー GO（推奨案B）** — 現行プール6名を世界観人物台帳に短プロフィールで登場。手引き／ニーナ露出はしない。名前は現行6を正とする。
| # | 決定 | 根拠 |
|---|---|---|
| P3-W-GACHA-NAMED-001-1 | **対象** — 招待状プール6（ヴァルデン／セリン／ミラ／イヴァル／カイダ／ガルム）。`_omitted` は載せない | 今後のガチャ実装土台 |
| P3-W-GACHA-NAMED-001-2 | **粒度** — `13_Characters` に短プロフィール（来歴・ゆかり・隊との関係）。★4ヴァルデンのみ問い付きで厚め | 初期5と同型・案B |
| P3-W-GACHA-NAMED-001-3 | **名称** — 現行6名を正とする。差替時は Glossary／`GachaHelperData` 同期 | 暫定のまま放置を解消 |
| P3-W-GACHA-NAMED-001-4 | **接続** — 一覧正は `08 §14` 据置。索引・Glossary・Q-06 を更新 | 運用ルール遵守 |
| P3-W-GACHA-NAMED-001-5 | **スコープ外** — 新キャラ追加／システム実装／手引き新条／ニーナ雑談への名前出し | 案B（文書のみ） |
...
## イベントバナーネームプレート統一（2026-07-25）
> **オーナー GO** — 岩角のみネームプレート付きだったのを全イベントへ。枠の太さはメイン Biome と同型（一重・約2px）。ダンジョン名ラベルをやや上へ。

| # | 決定 | 根拠 |
|---|---|---|
| P3-UI-EVT-BANNER-NP-001-1 | **全イベント BAN**（砂金／影狩り／ダック／レイヴン／岩角）に中央ネームプレート | オーナー指示・視認統一 |
| P3-UI-EVT-BANNER-NP-001-2 | **枠はメイン同型** — 一重金枠・約2px（手続二重枠は廃止） | オーナー指示 |
| P3-UI-EVT-BANNER-NP-001-3 | **名前ラベル** — Featured／一覧とも上寄せ（offset_top 6／bottom -10） | オーナー指示 |

**Closeout（2026-07-25）:** オーナー GO → 統合＋`main`。

## 在野助っ人追加・プール外データ（2026-07-24 — P3-GACHA-STAGED-001）
> **オーナー GO** — 既存6据置。追加は内部データのみ（招待状に出さない）。★4オーナー枠は別途。

| # | 決定 | 根拠 |
|---|---|---|
| P3-GACHA-STAGED-001-1 | **追加5体** — レノール★4／トルヴァ★3／シアン★3／ボルグ★2／ネリ★2 を `_omitted/` に配置 | 案GO・追加方針 |
| P3-GACHA-STAGED-001-2 | **排出プール不変** — 直下6体・排出率・天井据置 | 「ガチャには入れない」 |
| P3-GACHA-STAGED-001-3 | **台帳本載せは昇格時** — `13`／`08`／Glossary はプール投入 Task で同期 | プール外は `_omitted` 正 |
| P3-GACHA-STAGED-001-4 | **オーナー★4** — 未定義（オーナー設計待ち） | 指示どおり1体はオーナー |
...
| # | 決定 | 根拠 |
|---|---|---|
| P3-UX-LORE-002-1 | **通常判読率** — `SUCCESS_CHANCE` 0.5 → **0.8** | 案B・拾える体感の改善 |
| P3-UX-LORE-002-2 | **初回保証** — 図鑑「記録」が0件のときは判読必ず成功 | 「一度も開かない」を解消 |
...
| P3-ART-BS-ENM-001-2 | **ヴォイドテンタクル**（旧称・深海スクイッド／id=`abyssal_squid`）— **虚空の触腕群**（蛸頭巾ではない）。藍黒〜虚紫。濃い潮＋虚ろ霧。旗艦影・単独。深海司祭と差別化 | オーナー改名＋図鑑 |
...
## ④ブラックショア表示名変更（2026-07-25 — P3-ART-BS-NAME-002）
> **オーナー指示** — 表示名のみ。内部 id 据置。

| # | 決定 | 根拠 |
|---|---|---|
| P3-ART-BS-NAME-002-1 | **ロイヤルシャーク**（旧称・サムライフィッシュ／id=`samurai_fish`） | オーナー改名 |
| P3-ART-BS-NAME-002-2 | **深海司祭**（旧称・ニンジャオクトパス／id=`ninja_octopus`） | オーナー改名 |
| P3-ART-BS-NAME-002-3 | **スコープ外** — ステ／スキル／アート／id 変更なし | セーブ互換 |

## ④ブラックショア戦闘ドット差替（2026-07-25 — P3-ART-BS-DOT-001）
> **オーナー納品** — Desktop ブラックショアドット ZIP を `ENM_*_Sheet` / `BOSS_Nereion_Sheet` へ接続。id 据置。

| # | 決定 | 根拠 |
|---|---|---|
| P3-ART-BS-DOT-001-1 | 雑魚・Elite 9体＋ボス ネレイオンの戦闘シートを PixelLab ZIP から再生成（idle/attack/hurt/death） | オーナー納品 |
| P3-ART-BS-DOT-001-2 | `DungeonScene` 既存マップを維持（表示名変更済み id 含む） | 配線済み |
| P3-ART-BS-DOT-001-3 | **スコープ外** — 図鑑キーアート・ステ・スポーン変更なし | ドットのみ |

## Biome深層・無限エンド（2026-07-25 — P3-DG-ABYSS-001）
> **オーナー GO** — 報酬本命＝深層限定レジェンド（B）。マイルストーン33/66/99あり。99以降も無限延長。

エンドコンテンツ。メイン5 Biome 各1本。寄り道／征討とは別枠。

| # | 決定 | 根拠 |
|---|---|---|
| P3-DG-ABYSS-001-1 | **枠** — `kind=abyss`（仮）。仮ID `abyss_mourngate` 等5本。征討IDと衝突させない | 役割分離 |
| P3-DG-ABYSS-001-2 | **解放** — 当該 Biome ノーマル Boss クリア後 | 本編後のエンド帯 |
| P3-DG-ABYSS-001-3 | **難度** — 1–32≈N／33–65≈H／66–99≈NM／100+はNM超え緩増 | オーナー指定 |
| P3-DG-ABYSS-001-4 | **マイルストーン** — 33／66／99（初回厚・再到達薄） | 途中報酬あり |
| P3-DG-ABYSS-001-5 | **本命** — Biomeごと深層限定レジェンド1系統。初回99確定。以降低確率＋ソフト天井 | 案B |
| P3-DG-ABYSS-001-6 | **無限** — 99以降も継続。最高到達Fセーブ・途中退出可 | オーナー指定 |
| P3-DG-ABYSS-001-7 | **経済** — 通常報酬減衰。レジェンド量産禁止 | 本編・Hard/NMと共存 |
| P3-DG-ABYSS-001-8 | **スコープ外** — ランキング／メタ恒久バフ／個別レジェンド設計／β中解放 | 後続Task |
...
| # | 決定 | 根拠 |
|---|---|---|
| P3-DG-ABYSS-001-B-1 | 33初回=魔晶石8+epic5+王墓2／再=epic2 → **P3-BAL-ECO-001 で魔晶石100に上書き** | 案R（素材）＋経済是正 |
| P3-DG-ABYSS-001-B-2 | 66初回=魔晶石20+LB★3券+epic8+王墓3／再=魔晶石5+epic3 → **ECO-001 で200／再25** | R1 |
| P3-DG-ABYSS-001-B-3 | 99初回=レジェンド(C)+魔晶石30+王墓5／再=魔晶石10+王墓3 → **ECO-001 で300／再50** | 案R |
...
| # | 決定 | 根拠 |
|---|---|---|
| P3-BAL-ENEMY-SKILL-FIT-001-1 | ミラーボアに専用 `enemy_mirror_fang`（後列・毒） | ELITE無スキル穴を塞ぐ |
| P3-BAL-ENEMY-SKILL-FIT-001-2 | ゴールデンスカラベは `enemy_gold_dust_scatter`（金粉・slow）。晶毒針流用禁止 | テーマ整合 |
| P3-BAL-ENEMY-SKILL-FIT-001-3 | ②〜④メイン雑魚はロックバイソン以外 **最低1スキル** | バイオーム個性の底上げ |
| P3-BAL-ENEMY-SKILL-FIT-001-4 | 時環共鳴=thunder+**shock**、灰の帳=fire+**ignite** | 属性と状態の一致 |
| P3-BAL-ENEMY-SKILL-FIT-001-5 | 霜バイオーム敵の **ice 弱点は据置**（P3-D161-1 砕氷／氷晶ファンタジー）。火のみへの置換はしない | 再確認・明文化 |

## フロストリッジ弱点（2026-07-25 — オーナー指示）
> **オーナー指示** — フロストリッジの敵は基本 **火弱点**。
...
| # | 決定 | 根拠 |
|---|---|---|
| P3-BAL-ECO-001-1 | 通常クリア魔晶石 **50–100→35–65**。深層初回 **33=100／66=200／99=300**、再 **66=25／99=50**（素材・券は据置） | クリア＞深層の逆転を是正。招待500に対しクリア約10回相当 |
| P3-BAL-ECO-001-2 | 日課「20体撃破」に **Gold50＋魔晶石15**（遺跡結晶×2は維持） | 最頻日課の報酬感 |
| P3-BAL-ECO-001-3 | 供給レーン明文化＋微量強化 — エリート骨鉱 **25%**（素材優先35%）、ボス深層結晶 **45%**、Hard/NM雑魚に骨鉱 **10%/15%**。通常雑魚は基礎鉱／遺跡結晶のみ据置 | 中〜上級はエリート／ボス／深層主軸のまま詰まり緩和 |
| P3-BAL-ECO-001-4 | ゴールド・シンク — 炉研ぎ金 **50/80/120/180/280**、錬成 **30G/1gain** → **P3-BAL-FORGE-002** で +4/+5レア倍率・錬成帯倍率を追加 | 余り気味の供給を約1.5〜1.6倍シンクで吸収 |

**SSOT:** `12_AbyssMilestones.md`（魔晶石表）／`EquipmentEnhancer`／`DungeonController`／`daily_kill_enemies.tres`

---

## ガチャ経済（魔晶石）点検（2026-07-25 — P3-BAL-GACHA-001）

> **オーナー GO（A＋B1）** — 数値本体は ECO-001 据置。仕様ドリフト解消＋逆インセンティブ日課の除外。

| # | 決定 | 根拠 |
|---|---|---|
| P3-BAL-GACHA-001-A | **仕様同期（現行正）** — 単発 **500**／開始所持 **500**／Gold購入**なし**／天井**なし**／通常クリア **35–65**／深層初回 **100/200/300**（再66=25／99=50）／重複還元 ★2=50／★3=100／★4=150。旧 D036b「単発1・100G購入・天井30」は廃止扱い | 実装＝`GachaSystem`／`DungeonScene`／`AbyssMilestoneRewards`／`GachaRarityConfig` |
| P3-BAL-GACHA-001-B1 | **日課 `daily_gacha_pull` を抽選プールから除外**（`.tres` は残置）。当日セーブに残っていれば再抽選 | 消費500に対し報酬20の逆インセンティブ解消 |
...
> **オーナー指定 GO** — はじめから初回のみ。拠点上の大きめポップ＋右下ニーナ・ドット。6ページ各1P。完了後ジャック支給＋魔晶石500。

| # | 決定 | 根拠 |
|---|---|---|
| P3-UI-HUB-GUIDE-001-1 | **6ページ** — ①招待状で仲間 ②DGで装備・Lv ③調査室 ④鍛冶 ⑤展示室 ⑥ジャック支給。各1P | オーナー指定 |
...
| P3-UI-HUB-GUIDE-001-5 | **完了儀式** — ジャック加入（鳴き声）→ 魔晶石500飛込。ガイド終了まで魔晶石0・ジャック未所持 | オーナー指定 |
...
## 罠の難易度ティア別化（2026-07-27 — P3-BAL-TRAP-TIER-001）

> **オーナー GO（案A）** — 非戦闘が厳しすぎるため、罠を N/H/NM で段階化。ノーマル緩和／上位で厳しく。

| # | 決定 | 根拠 |
|---|---|---|
| P3-BAL-TRAP-TIER-001-1 | **発動** — 部屋 50/65/80%・探索 12/20/28%・全体 25/35/45% | 難易度上昇で踏まれやすく |
| P3-BAL-TRAP-TIER-001-2 | **ダメ** — 部屋単体 15/25/35%・部屋全体 8/12/18%・探索単体 10/15/22%・探索全体 5/8/12% | ハード≈旧据置 |
| P3-BAL-TRAP-TIER-001-3 | **状態異常** — N=なし／H=毒or出血40%／NM=60%。被弾対象ごと | 上位の痛み |
| P3-BAL-TRAP-TIER-001-4 | **据置** — イヴァル免疫・罠解除。宝箱／泉／碑文の失敗は今回対象外 | スコープ限定 |

**SSOT:** `docs/specs/decisions/10_NonCombatTension.md` §1
...
| # | 決定 | 根拠 |
|---|---|---|
| P3-UX-STATUS-LEGEND-001-1 | **位置** — 戦闘フィールド右上（旧敵行動順） | 空きスペース活用 |
| P3-UX-STATUS-LEGEND-001-2 | **対象** — 味方＋敵に現在かかっている状態のユニーク集合。0件なら非表示 | 「今発生している」 |
| P3-UX-STATUS-LEGEND-001-3 | **行** — アイコン＋主効果1行（持続／重ね省略） | 可読性 |
| P3-UX-STATUS-LEGEND-001-4 | **DoT文言** — 「継続ダメージ」（「刻ごと」禁止）。※旧「1秒ごと」は CT 間隔の平易語だったが壁時計誤解のため 2026-08-02 訂正 | オーナー確認 |
| P3-UX-STATUS-LEGEND-001-5 | **スコープ外** — 誰に付いているかの個別表示／タップ詳細 | MVP |
| P3-UX-STATUS-LEGEND-001-6 | **行文言** — `表示名:効果`（例: `毒:継続ダメージ`） | どの状態か分かるように |
| P3-UX-STATUS-LEGEND-001-7 | **毒 DoT 数字色** — 紫系（緑禁止。回復 +N と被る） | 2026-08-02 オーナー |

**Closeout:** Impl 後に統合＋`main`。

---

## 戦闘右上レジェンドに天候行（2026-07-30 — P3-UX-STATUS-LEGEND-002）

> **オーナー GO（推奨）** — 天候などラン環境効果を状態異常説明の最上位に、同型のアイコン＋1行で表示。

| # | 決定 | 根拠 |
|---|---|---|
| P3-UX-STATUS-LEGEND-002-1 | **対象** — 天候のみ（地形有利はオミット維持） | 現状レジェンド未接続は天候 |
...
| # | 決定 | 根拠 |
|---|---|---|
| P3-UX-SURVEY-CANCEL-001-1 | **対象** — 進行中の DISPATCH（短／標準）のみ。完了済みは「受け取る」必須 | 報酬チート防止 |
| P3-UX-SURVEY-CANCEL-001-2 | **効果** — サイクル破棄。報酬0・SURVEY加算0・日次cap非消費。隊員は調査から解放 | リタイアと同型 |
| P3-UX-SURVEY-CANCEL-001-3 | **UI** — 調査中のみ「調査を中止」＋確認ダイアログ | 誤操作防止 |
| P3-UX-SURVEY-CANCEL-001-4 | **対象外** — 完全調査100%景品（瞬間清算）／ギルド情報誌 FieldSurvey | 別系統 |

**Closeout:** Impl 後に統合＋`main`。

---

## 結果画面・全滅敗因分析（2026-07-27 — P3-UX-WIPE-CAUSE-001）

> **オーナー GO（推奨案A）** — 全滅時のみ、報酬ステップ先頭に1〜2行の敗因要約。

| # | 決定 | 根拠 |
|---|---|---|
| P3-UX-WIPE-CAUSE-001-1 | **対象** — `last_run_outcome=wipe` のみ。クリア／リタイアは非表示 | 敗北時の学び |
| P3-UX-WIPE-CAUSE-001-2 | **1行目** — `{階層}・{部屋種}で全滅`（罠は罠表記） | どこで落ちたか |
| P3-UX-WIPE-CAUSE-001-3 | **2行目** — 戦闘なら `主因: {敵名}`。なければ被ダメ／回復からヒント1つ | → **002 で上書き** |
| P3-UX-WIPE-CAUSE-001-4 | **データ** — `last_run_wipe_cause` を wipe 直前にスナップショット | ラン揮発 |
| P3-UX-WIPE-CAUSE-001-5 | **スコープ外** — 死亡順・スキル名・詳細カード・攻略強制 | → 詳細行は **002**。死亡順・スキル名・カードは継続外 |
...
| # | 決定 | 根拠 |
|---|---|---|
| P3-UX-SKILL-LEARN-PERSIST-001-1 | **位置** — 経験値行のキャラ名（Lv）の横 | オーナー指定 |
| P3-UX-SKILL-LEARN-PERSIST-001-2 | **文言** — 「{スキル名}を習得！」（名前は行頭にあるため主語省略） | 横並びの可読性 |
| P3-UX-SKILL-LEARN-PERSIST-001-3 | **永続** — フェードアウトしない。スキップ後も最終習得を表示 | 「ずっと残す」 |
| P3-UX-SKILL-LEARN-PERSIST-001-4 | **複数** — 同一ランで複数解放時は最後に習得したスキル名に更新 | オーナー指定 |
| P3-UX-SKILL-LEARN-PERSIST-001-5 | **廃止** — 画面中央の一過性ポップアップ | 重複・消えやすさ |
...
| # | 決定 | 根拠 |
|---|---|---|
| P3-SHOWCASE-ONLINE-001-1 | **β範囲外** — 展示室は「自分の展示」「スタッフ作例」のまま | サーバー基盤が未整備 |
| P3-SHOWCASE-ONLINE-001-2 | **将来案** — 「みんなの展示」タブ（自分とスタッフの間）＋自慢キャラのサーバー公開／閲覧 | オーナー要望 2026-07-26 |
| P3-SHOWCASE-ONLINE-001-3 | **仮UI禁止** — 空タブ／準備中だけの先行配置はしない | 体験が薄くなるため |
| P3-SHOWCASE-ONLINE-001-4 | **実装条件** — アカウント／公開ペイロード／荒らし対策／オフラインUX を Decision してから Task 化 | アーキテクチャ依存 |
...
| P3-UX-COMBAT-BAND-ART-001-2 | **未配置＝無演出**（ヒット／❗／カットイン／既存リングは据置） | 四角に戻さない |
...
| # | 決定 | 根拠 |
|---|---|---|
| P3-UX-EQUIP-NEW-001-1 | **New** — ダンジョンドロップ装備はキャラ装備・装備一覧のアイコン中央に `New` を点滅表示 | 入手直後の識別 |
| P3-UX-EQUIP-NEW-001-2 | **消込** — 次のダンジョン潜行開始（`start_dungeon`／`_reset_run_state`）で New を消す | 「次に潜るまで」 |
| P3-UX-EQUIP-NEW-001-3 | **対象外** — 鍛冶作成・調査室景品は New にしない | 「ドロップ」限定 |
| P3-UX-EQUIP-NEW-001-4 | **装** — 装備中を示す左下「装」文字を削除（キャラ／装備一覧） | ノイズ削減 |

---

## 結果・入手装備一覧（2026-07-27 — P3-UX-RESULT-DROP-LIST-001）

> **オーナー GO（案A）** — 結果の報酬ステップ内に、ラン中ドロップ装備の全件グリッド。
...
## 天候弱体＋炎天／吹雪追加（2026-07-29 — P3-WEATHER-W1-A-001）

> **オーナー GO（推奨案 W1＋A）** — 既存天候の補正を弱体し、炎天／吹雪を追加。天候本格（時間帯変化・敵側補正・Biome別・選択画面事前表示）は据置。

| # | 決定 | 根拠 |
|---|---|---|
| P3-WEATHER-W1-A-001-1 | **W1** — 属性補正 +15%→**+10%**、−10%→**−5%**。霧 ×0.95→**×0.97**（与ダメ・被ダメとも） | 天候の過剰干渉を抑えつつ属性選択の意味は残す |
| P3-WEATHER-W1-A-001-2 | **A** — 新規 `heat`（炎天）=炎+10%/氷−5%、`snow`（吹雪）=氷+10%/炎−5% | 炎／氷ビルドの天候軸を雨・夜と対称に |
| P3-WEATHER-W1-A-001-3 | **抽選重み** — 晴れ45／雨・夜・霧・炎天・吹雪 各11（合計100） | 晴れ多めを維持しつつ6種へ再配分 |
| P3-WEATHER-W1-A-001-4 | **野外スロット** — `weather_heat` / `weather_snow` を既存天候と同重みで追加 | 情報誌固定とラン抽選を揃える |
...
## 属性レジェンド個性化（2026-07-30 — P3-BAL-LEG-ATTR-B001）

> **オーナー GO（ビルド点検案B）** — 属性確定＋平％で似通ったレジェンドを、数を増やさず条件付き個性へ差し替え。天候／深層／穴埋めLは据置。

| # | 決定 | 根拠 |
|---|---|---|
| P3-BAL-LEG-ATTR-B001-1 | **方針** — 平属性％を下げ、状態シナジー／列位置へ寄せる（新規枠なし） | ビルド点検案B |
| P3-BAL-LEG-ATTR-B001-2 | **ヴォルグレイヴ** — 雷+25% → 雷+10%＋ショック敵+40%＋攻撃25%ショック | 雷DoT／状態核 |
| P3-BAL-LEG-ATTR-B001-3 | **エルディオン・ブランド** — 氷+25% → 氷+10%＋冷却敵+40%＋攻撃25%冷却 | 氷状態核（剣） |
| P3-BAL-LEG-ATTR-B001-4 | **シルヴァリア・ファング** — 炎+25% → 炎+10%＋炎上敵+40%＋攻撃25%炎上 | 炎状態核 |
| P3-BAL-LEG-ATTR-B001-5 | **エルディオン・スパイン** — 氷+25%/後列通常+15% → 氷+10%/後列+25%＋冷却敵+20% | 後列弓の個性を前面に |
| P3-BAL-LEG-ATTR-B001-6 | **エルディオン・クロー** — 氷+15%/3撃冷却 → 氷+5%＋冷却敵+30%＋3撃冷却据置 | 連撃冷却のリズム核 |
| P3-BAL-LEG-ATTR-B001-7 | **氷鱗鎧** — 被弾冷却のみ → 被ダメ-8%＋被弾冷却（潮鎧との被り削減） | 防具側の同型整理 |
| P3-BAL-LEG-ATTR-B001-8 | **据置** — 天候シンクロ／深層／会心・必殺・タンク穴埋めL | 既に条件付き個性 |
| P3-BAL-LEG-ATTR-B001-9 | **後続** — レリック職接続（案C）／開幕装飾整理（案D） | スコープ分離 |

## レリック職接続＋開幕装飾整理（2026-07-30 — P3-BAL-RELIC-ACC-CD-001）

> **オーナー GO（ビルド点検案C＋案D）** — 新枠なし。レリックを薄い軸の核へ寄せ、開幕小鼓舞の重複を別役割へ。
...
| # | 決定 | 根拠 |
|---|---|---|
| P3-BAL-RELIC-ACC-CD-001-D1 | **王家の覇気** — 自身小鼓舞 → 初撃×1.75 | 個人開幕火力 |
...
## 助っ人パッシブ性能・ビルド接続（2026-07-30 — P3-BAL-HELPER-PASSIVE-B001）

> **オーナー GO（点検1→3）** — ★序列とビルド接続の是正。ヴァルデン／火鷹等は据置。
...
## 調査許可等級の到達速度・報酬・TopBar進捗（2026-07-30 — P3-CMD-RANK-REWARD-001）

> **オーナー GO** — 報酬薄い／到達が早すぎる／名前横（下）に経験値風バー。推奨パッケージ一括。

| # | 決定 | 根拠 |
|---|---|---|
| P3-CMD-RANK-REWARD-001-1 | **閾値** — D0 / C200 / B700 / A1500 / S2800（SP重み据置） | 案A。計算式はそのまま |
| P3-CMD-RANK-REWARD-001-2 | **既存セーブ据置** — 旧閾値での到達等級を下限に（表示等級は下がらない）。`rank_curve_v2` で一度だけ移行 | 違和感回避 |
...
## 戦闘レジェンド分割（状態＝右下／ダンジョン効果＝右上）（2026-07-31 — P3-UX-STATUS-LEGEND-006）

> **オーナー** — 状態異常アイコン説明は右下のまま。気候などダンジョンにかかる効果のみ右上。
...
## 神話武器4種・歴史名（2026-07-30 — P3-EQ-MYTHIC-WPN-TYPES-001）

> **オーナー GO** — 剣以外（弓／杖／双剣）を追加。名前は九対／九英雄に接続。真偽は未確定フレーミング。

| # | 決定 | 根拠 |
|---|---|---|
| P3-EQ-MYTHIC-WPN-TYPES-001-1 | **武器4種** — 剣／弓／杖／双剣を各1本（防具・装飾は据置） | 全職が神話武器ルートを持てる |
| P3-EQ-MYTHIC-WPN-TYPES-001-2 | **名前** — 継承剣レガート／巡礼弓ルーメン／叡智の杖ノエシス／深淵双牙リュシアン | 歴史正典接続。双剣は九対外＝九英雄リュシアン |
| P3-EQ-MYTHIC-WPN-TYPES-001-3 | **剣改名** — 旧「葬冠の大剣」→表示名「継承剣レガート」。`id=burial_crown_greatsword` 据置（セーブ互換） | モーン＝アステリア地下と継承伝承が最短 |
| P3-EQ-MYTHIC-WPN-TYPES-001-4 | **フレーバー** — 記録部は伝承名で仮登録。**真の王遺産かは未確定** | 九対「ほぼ未確認」設定と衝突させない |
| P3-EQ-MYTHIC-WPN-TYPES-001-5 | **効果** — 剣据置（+25%／撃破CT）。弓+20%＋標的特効／杖+15%＋スキル威力／双剣+15%＋出血付与 | 種ごとに別ルール・同帯 |
| P3-EQ-MYTHIC-WPN-TYPES-001-6 | **入手** — モーンボス再クリア1%プールに武器3本追加（計6枠）。未所持優先据置 | β内完結 |
| P3-EQ-MYTHIC-WPN-TYPES-001-7 | **アイコン** — 仮流用PNG。固有アートは後続 | 出荷止めない |

## 生産＝入手解放・N〜★（神話除外）（2026-07-31 — P3-CRAFT-DISCOVER-001）

> **オーナー GO（推奨パッケージ全採用）** — 生産を「拾った装備を狙って作る」特化へ。探索ドロップ本線は据置。

| # | 決定 | 根拠 |
|---|---|---|
| P3-CRAFT-DISCOVER-001-1 | **解放** — 装備マスタを一度入手したらレシピ永続解放（`unlocked_craft_outputs`）。一覧は解放済みのみ | 発見感／未知レシピを並べない |
| P3-CRAFT-DISCOVER-001-2 | **対象レア** — N〜LEGENDARY まで生産可。コストはレア別（★は重い: Gold400＋王墓欠片2 等） | 案C′＋L1 |
| P3-CRAFT-DISCOVER-001-3 | **除外** — MYTHIC／SET（降臨）／イベント専売は生産不可 | 希少性の本線を守る |
| P3-CRAFT-DISCOVER-001-4 | **データ** — 固定6 `.tres` 依存をやめ、解放済みマスタから動的に CraftData 相当を生成 | スケール |
...
## ボス圧力（詠唱全体の薄さ）（2026-07-31 — P3-BAL-BOSS-PRESSURE-001）

> **オーナー GO（F1＋F2・セルディオン先行）→ 横展開 GO** — 4人パーティでボス行動前に削られる主因＝全体が詠唱付き＋スキル率40%＋激昂枠。

| # | 決定 | 根拠 |
|---|---|---|
| P3-BAL-BOSS-PRESSURE-001-1 | **F1** — `enemy_serdion_roar`（骸晶の咆哮）を **即時全体**（`cast_time=0`）。倍率0.5・CD6。断罪の波動は詠唱1維持（演出用） | 着弾前に倒される感を減らす |
| P3-BAL-BOSS-PRESSURE-001-2 | **F2** — セルディオン `skill_use_chance` ベース0.55。フェーズ重みで激昂を下げ、咆哮／断罪を上げる | 通常攻撃偏重・自己バフ偏重を抑える |
| P3-BAL-BOSS-PRESSURE-001-3 | **スコープ（更新）** — **全ボス＋全エリートへ横展開**（オーナー指示） | ①検証後の本線適用 |
| P3-BAL-BOSS-PRESSURE-001-4 | **ボス F1** — 圧力用全体（軽い方）を即時化・倍率微増・CD6。二重全体の重い方／演出用は詠唱維持（例: 炉床震動／外洋の慟哭） | セルディオン同型 |
| P3-BAL-BOSS-PRESSURE-001-5 | **ボス F2** — ベース0.55・フェーズ0.55/0.65/0.75・全段 `skill_weight` で激昂＜全体／単体圧力。Hard/NM 0.60/0.65（既存 tier あり分） | セルディオン同型 |
| P3-BAL-BOSS-PRESSURE-001-6 | **エリート** — ベース0.45・Hard/NM 0.50/0.52（既存 tier）。`enemy_chrono_resonance` のみ詠唱→即時（他エリートAoEは既に即時） | ボスより一段弱め |
...
| # | 決定 | 根拠 |
|---|---|---|
| P3-FORGE-REFORGE-001-1 | **対象** — 武器のみ（防具・装飾は後続）。鑑定済み必須 | 指示どおり／炉研ぎと同前提 |
| P3-FORGE-REFORGE-001-2 | **操作** — 強化タブで武器選択 → `random_mods` 1枠選択 →「焼直し」 | 当たり直し導線 |
| P3-FORGE-REFORGE-001-3 | **再抽選** — 枠ごと（kind＋数値）。他枠の kind は重複禁止。選択中枠の kind は再候補に含めてよい | ドロップ時と同型 |
| P3-FORGE-REFORGE-001-4 | **除外** — 生態特効（bane）は再抽選不可。属性値は数値のみ再ロール（属性種類はマスタ固定） | マスタ固定枠を壊さない |
| P3-FORGE-REFORGE-001-5 | **炉研ぎ独立** — `enhance_level` 無関係。+5 でも焼直し可 | 上限後も個体調整できる |
...
## 分解返却増（2026-07-31 — P3-BAL-DISMANTLE-001）

> **オーナー GO（推奨: A＋B＋◇基礎＋1）** — 分解素材が少なすぎる。主因は生産解放後の60%キャップ `floor` が小コスト素材を0にする点。

| # | 決定 | 根拠 |
|---|---|---|
| P3-BAL-DISMANTLE-001-1 | **キャップ修正** — レシピに含まれる素材の上限は `max(1, floor(コスト×比率))` | ゼロ潰し解消（案A） |
| P3-BAL-DISMANTLE-001-2 | **比率** — 60% → **80%** | 案B。往復はまだ割に合わない |
...
## 灰冠限定防具・装飾18（2026-07-31 — P3-GACHA-EQ-KAIWAN-003）

> **オーナー GO** — 席タグ共有。防＝変換／飾＝増幅。二つ名＋カタカナ。

| # | 決定 | 根拠 |
|---|---|---|
| P3-GACHA-EQ-KAIWAN-003-1 | **防具9** — 塞図プライムハイド／売境ブラッドメイル／裂鍵ヴォイドローブ／違約オースブレイク／灯断ダスクメイル／偽星フォージペイト／奪潮タイドスキン／枯翠ソーンメイル／断継ラストコイル | オーナー GO |
| P3-GACHA-EQ-KAIWAN-003-2 | **装飾9** — 塞図イニシオ／売境ヴェノムバンド／裂鍵アンロック／違約カースバンド／灯断ノクターン／偽星スパークル／奪潮リーフフック／枯翠ウィザー／断継ネクストエッジ | オーナー GO |
...
## 灰冠限定ランダムステ（2026-07-31 — P3-GACHA-EQ-KAIWAN-004）

> **オーナー GO** — 全て推奨値。通常プール排除・席保証1・共通12・二面ロールなし。

| # | 決定 | 根拠 |
|---|---|---|
| P3-GACHA-EQ-KAIWAN-004-1 | **分離** — 灰冠 L は通常 `random_mods` プールを使わない | ビルド装備の顔 |
| P3-GACHA-EQ-KAIWAN-004-2 | **枠** — L＝4。枠1＝自席共鳴保証／枠2〜4＝灰冠共通。他席共鳴なし | 席ビルドをぼかさない |
| P3-GACHA-EQ-KAIWAN-004-3 | **席共鳴9** — 初撃与ダメ／痛撃変換／解呪威力／デバフ特攻／開幕代償火力／強化追撃／疾風潮撃／転火力／撃破連鎖 | 推奨採用 |
| P3-GACHA-EQ-KAIWAN-004-4 | **共通12** — 影刃／灰心／灰滅／呪付与／呪威／後列射／薄ガラス／追い灰／先血／灰迅／自傷儀式／無慈悲 | 推奨採用 |
...
## 下ナビ短名を「召喚」へ（2026-07-31 — P3-GACHA-NAV-SUMMON-001）

> **オーナー GO（推奨A）** — 封蔵追加後、下ナビ「招待状」は画面全体を指さない。

| # | 決定 | 根拠 |
|---|---|---|
| P3-GACHA-NAV-SUMMON-001-1 | **下ナビ／左メニュー** — 短名 **「召喚」**（招き＋封蔵の総称） | オーナー採択 |
| P3-GACHA-NAV-SUMMON-001-2 | **据置** — 画面内「ギルドへの招待状」「封蔵の匣」「招待状を開く」等のページ語彙 | 招きページの世界観 |
...
| # | 決定 | 根拠 |
|---|---|---|
| P3-FIX-SURVEY-AUDIT-A-001-1 | **編成復元** — 開始時に `party_ids_before` 保存。受取／中止で復元してから EXP 付与。**完了（受取待ち）時点でも自動復帰**（`party_restored`）。**受取は常に派遣前へ復帰**（フラグ付き欠員のスキップ禁止）。調査室進捗でも ensure | 中止文「戻ります」と不一致。編成外だと本人 EXP パッシブ不発。❗️／受取後の欠員確定事故 |
...
## 敵トリッキー第2波（2026-08-02 — P3-BAL-ENEMY-TRICKY-005）

> **オーナー GO（推奨）** — T8召集／T10沈黙／T11吸血／T14加速。段階A→C。SSOT=`docs/specs/decisions/36_EnemyTrickyWave2.md`。
...
## 全滅敗因分析・詳細化（2026-08-02 — P3-UX-WIPE-CAUSE-002）

> **オーナー GO（推奨案A）** — 現行1〜2行を **2〜3行** に強化。攻略強制文は出さない。

| # | 決定 | 根拠 |
|---|---|---|
| P3-UX-WIPE-CAUSE-002-1 | **1行目** — `{階層}・{部屋種}で全滅`（001-2 据置） | どこで落ちたか |
| P3-UX-WIPE-CAUSE-002-2 | **2行目** — `主因: {敵名}`。群れ2体以上は `（N体）` | 状況の密度 |
| P3-UX-WIPE-CAUSE-002-3 | **3行目（任意）** — ヒント最大2種を `／` 結合。優先＝集中攻撃→回復不足→残存状態異常→天候 | 学びの厚み |
| P3-UX-WIPE-CAUSE-002-4 | **スナップ** — `enemy_count`／`weather_id`／`party_status_ids` を wipe 直前に追加 | ヒント材料 |
| P3-UX-WIPE-CAUSE-002-5 | **スコープ外** — 死亡順・致死スキル名・詳細カード・攻略強制 | 記録未整備／説教回避 |
| P3-UX-WIPE-CAUSE-002-6 | **上書き** — `001-3` の「ヒント1つ／膨らませない」 | 本 Decision が正 |
...
## 長期戦怒涛＋硬殻バースト（2026-08-09 — P3-BAL-COMBAT-ATTRITION-001）

| ID | 決定 | 理由 |
|---|---|---|
| P3-BAL-COMBAT-ATTRITION-001-1 | **怒涛** — grace 18CT 後、12CT ごとに敵与ダメ +12%（上限 +60%） | 回復ドレイン持久戦の締め |
...
| P3-BAL-ULTIMATE-TIME-001-2 | **満タン** — 通常 **50秒**（`ULTIMATE_CHARGE_FILL_SECONDS`・当初25→半速）。上限100据置 | オーナー「まだ早すぎる」で半速化 |
...
## セルディオン通常攻撃2種＋断罪強化（2026-08-02 — P3-BAL-SERDION-BASIC-001）

> **オーナー指示** — ボス弱体感の是正。通常＝単体1.5／全体1.0。断罪＝2.0。

| # | 決定 | 根拠 |
|---|---|---|
| P3-BAL-SERDION-BASIC-001-1 | **通常攻撃** — `basic_attack_skill_ids` で均等抽選。骸晶爪（単体×1.5・出血20%）／骸晶薙ぎ（全体×1.0） | オーナー指定 |
| P3-BAL-SERDION-BASIC-001-2 | **断罪の波動** — `power_multiplier` **0.6→2.0**（詠唱1・CD8据置） | オーナー指定 |
| P3-BAL-SERDION-BASIC-001-3 | **配線** — 通常バリエーションは `skill_ids` 外。CD非消費でスキルダメージ経路 | スキル枠と二重計上しない |
| P3-BAL-SERDION-BASIC-001-4 | **据置** — 咆哮／威圧／激昂／開幕オーラ／フェーズ重み | スコープ最小化 |

## 鍛冶 Gold 重化＋高レア強化（2026-08-02 — P3-BAL-FORGE-GOLD-HEAVY-001）

> **オーナー指示** — 鍛冶 Gold 全般を重く。強化はレア補正。L／エンシェント／ミシックは1回相当重く。

| # | 決定 | 根拠 |
|---|---|---|
| P3-BAL-FORGE-GOLD-HEAVY-001-1 | **炉研ぎ基本** — +1〜+5 = **100/160/240/360/560** | 旧表の約2倍 |
| P3-BAL-FORGE-GOLD-HEAVY-001-2 | **レア倍率（全段）** — ◇1.0／◆1.25／✦1.6／★**3.0**／エンシェント(SET)**3.5**／ミシック**4.0** | 高レア1段を特に重く。旧 +4/+5 のみ倍率は廃止 |
| P3-BAL-FORGE-GOLD-HEAVY-001-3 | **錬成** — `60 × 上昇 × 帯倍率`（帯は FORGE-002 据置）→ **P3-BAL-ALCHEMY-GOLD-AB-001 で上書き** | 全般シンク |
| P3-BAL-FORGE-GOLD-HEAVY-001-4 | **焼直し** — ◇100／◆160／✦250／★500／エンシェント700／ミシック800 | 全般＋高レア |
...
## 錬成 Gold A+B（2026-08-07 — P3-BAL-ALCHEMY-GOLD-AB-001）

> **オーナー GO（A+B）** — 単価と帯倍率の両方を重く。

| # | 決定 | 根拠 |
|---|---|---|
| P3-BAL-ALCHEMY-GOLD-AB-001-1 | **単価** — `120 × 実上昇`（旧 60） | 案A |
| P3-BAL-ALCHEMY-GOLD-AB-001-2 | **帯倍率（素材Lv）** — **1–20×1.5／21–50×2.0／51+×3.0**（旧 1.0／1.5／2.0） | 案B |
| P3-BAL-ALCHEMY-GOLD-AB-001-3 | **式** — `ceil(120 × 実上昇 × 帯)`。レア倍率は無し（炉研ぎのみ） | 据置 |
| P3-BAL-ALCHEMY-GOLD-AB-001-4 | **上書き** — HEAVY-001-3／FORGE-002-2 の錬成 Gold | 本 Decision が正 |
...
> **2026-08-04 更新** — エンシェント／ミシック不使用（レジェンド以下の役割装備へ）。

| # | 決定 | 根拠 |
|---|---|---|
| P3-SHOWCASE-IDEAL-001-1 | **5枠** — アルド出血主砲／ガレン防壁盾役／セリン野営ヒーラー／リーヴァ標的シナジー／ミレイペット指揮 | 役割カタログ |
| P3-SHOWCASE-IDEAL-001-2 | **名札** — `{キャラ名}({ビルド名})`。旧「スタッフN-名前」廃止 | オーナー指定 |
| P3-SHOWCASE-IDEAL-001-3 | **装備レア** — レジェンド以下のみ。エンシェント（SET）／ミシック不可（2026-08-04 更新。旧: 降臨セット可） | オーナー指定 |
...
## ボス召喚／エルディオン再生（2026-08-03 — P3-BAL-BOSS-SUMMON-REGEN-001）

> **オーナー指示** — グランヴェル／モルドガル／ネレイオン／クロノス・ウェーブに一度限りの召喚。エルディオンに徐々に回復。
...
| P3-DG-FLOOR-CHOICE-001-2 | **本編**≤9Fは1回・10Fは2回。**無限**は10Fごとに1回。効果は次の1フロアのみ | オーナー更新（旧: ≤6は1／章最大2） |
| P3-DG-FLOOR-CHOICE-001-3 | **A 戦力**＝与ダメ×1.5＋被ダメ×0.85（案A・2026-08-06 更新）。**A′ 応急**＝消耗時のみ（平均HP&lt;60% or 戦闘不能）次F開始25%回復 | ピンチ保険／攻守のステータス強化 |
...
| # | 決定 | 根拠 |
|---|---|---|
| P3-BAL-BT-RG-KIT-TUNE-001-1 | **獣医の手当て**→表示名 **血還の矢** — ドレイン攻撃（敵×**1.1**／自己＝与ダメ**50%**／CD9.5／**即時** ※詠唱1は誤残→撤去） | サポート攻撃の役割明確化／オーナー改名B |
...
| P3-UX-EQUIP-LOCK-001-2 | **UI** — **装備一覧のみ**長押しでトグル（短押し＝詳細のまま）。右下🔒。詳細に「ロックする／解除」。**キャラ画面ではロック操作なし**（表示のみ） | 着脱とロック操作を分離 |
...
## 職テーマパッシブ実装 GO（2026-08-08 — P3-BAL-JOB-THEME-001 実装）

> **オーナー GO** — 差し替え数値確定を実装。

| # | 決定 | 根拠 |
|---|---|---|
| P3-BAL-JOB-THEME-001-9 | エリアス＝万象の触媒・属性与ダメ×1.20 | オーナー |
| P3-BAL-JOB-THEME-001-10 | シアン＝沈黙の罠糸・28%で冷却／鈍化／恐怖 | オーナー |
| P3-BAL-JOB-THEME-001-11 | ミレイ＝毒牙・28%で毒／出血／炎上 | オーナー |
...
| # | 決定 | 根拠 |
|---|---|---|
| P3-BAL-ELDION-OPENING-001-1 | **開幕同席なし**（旧×2→×1→撤廃） | オーナー指示 |
...
> **オーナー指示** — 降臨ボスからドロップするエンシェント装備は1個固定（＝一度にいっぱい出さない）。

| # | 決定 | 根拠 |
|---|---|---|
| P3-BAL-DESCENT-SET-DROP-ONE-001-1 | **数量上限** — 撃破1回あたりエンシェント（SET）装備 **最大1個** | 一度に複数ドロップしない |
| P3-BAL-DESCENT-SET-DROP-ONE-001-2 | **初回** — 当該ティア初回は **1個確定**（未所持優先） | 旧3部位一括を廃止 |
| P3-BAL-DESCENT-SET-DROP-ONE-001-3 | **再周回** — **40%で1個**（据置） | 「毎回確定」ではない |
...
| P3-JOB-ENGINEER-001-6 | スキル7＝スパイク／ドリル／スネア／チャージ／ブレイク／スクラップ／バーン | 罠3・破砕1・装炎2・拡張1 |
| P3-JOB-ENGINEER-001-7 | ブレイクトラップ＝敵行動で物理＋甲砕（旧マルチ不採用） | オーナー指定 |
...
## 機巧士やや強化（2026-08-31 — P3-JOB-ENGINEER-BAL-001）

> **オーナー GO（推奨パッケージ）。** 他職より意図的に少し強く。罠税＋破砕シナジー。基礎ATK据置。

| # | 決定 | 根拠 |
|---|---|---|
| P3-JOB-ENGINEER-BAL-001-1 | スパイク作動 **ATK×0.55→0.65**（残発4据置） | 罠の税を厚く |
| P3-JOB-ENGINEER-BAL-001-2 | **甲砕中**の敵への仕掛け作動 **×1.15**（全kind・職固有） | 破砕→罠の型が得 |
| P3-JOB-ENGINEER-BAL-001-3 | 基礎ステ・必殺・装炎・印上限は据置 | 「少し強い」に留める |
| P3-JOB-ENGINEER-BAL-001-4 | SSOT — `126` §5.8／`EngineerTraps` | Decision 本体 |
...
## エンシェント地力＋単品（2026-08-31 — P3-EQ-ANCIENT-POWER-D-001）

> **オーナー GO**（「DでGo」）。地力を中L手前まで＋薄い単品パッシブ。**3部位加護は据置**。

| # | 決定 | 根拠 |
|---|---|---|
| P3-EQ-ANCIENT-POWER-D-001-1 | 武器 ATK 目安 ×約2.2（時環／名拒み／星炉。アンティークはやや低め） | レジェンド3点負け緩和 |
