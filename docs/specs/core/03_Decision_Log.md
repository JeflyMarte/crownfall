...
| P3-CHR-OMIT-001-2 | **`GACHA_HELPERS_PLAYABLE=false`** — 召喚所ロック・抽選不可・ロスターから `gacha_*` 除外。`gacha_helpers/` データと `owned_helpers` セーブは残置 | P3-DG-OMIT と同型。再有効化で復帰可 |
| P3-CHR-OMIT-001-3 | ゲーム本編のプレイ対象キャラ＝**スターター5人のみ**（ジョブは従来どおり5職） | ドット一新の対象範囲を明確化 |
...
## Git / Mac / Cloud 同期運用（2026-07-12 — P3-OPS-GIT-001）

> **オーナー GO** — Mac ローカルと Cursor Cloud Agent の巻き戻り・未同期を防ぐ。オーナー指示は開始フレーズ2種に簡素化し、エージェントが SSOT 手順に変換する。

| # | 決定 | 根拠 |
|---|---|---|
| P3-OPS-GIT-001-1 | **オーナー開始指示は2種のみ** — 「遠隔で作業開始」「MACで作業開始」。詳細 git 操作はエージェントが `07_Git_Mac_Cloud_Sync.md` に従い実行 | 指示の簡素化 |
| P3-OPS-GIT-001-2 | **Mac 正パス** — `/Users/marte/Projects/crownfall`。Godot と git cwd を一致 | フォルダ混在事故防止 |
| P3-OPS-GIT-001-3 | **遠隔ブランチ** — `cursor/<slug>-cca2`。統合確認用に一時統合ブランチ可（例: `cursor/sub-mac-ui-integration-cca2`） | Cloud 1 Task 1 ブランチ運用 |
| P3-OPS-GIT-001-4 | **遠隔終了時** — commit+push+PR 更新に加え、Mac 用 pull ブロックを必ず提示 | Mac 未同期防止 |
| P3-OPS-GIT-001-5 | **stash** — `git-wip-safety.mdc` 準拠。オーナー確認なし drop 禁止 | cloud agent 退避事故 |
| P3-OPS-GIT-001-6 | **SSOT** — `docs/specs/core/07_Git_Mac_Cloud_Sync.md` + `.cursor/rules/git-mac-cloud-sync.mdc` | エージェント常時適用 |
| P3-OPS-GIT-001-7 | **変換ルール** — SSOT §4 に T-REMOTE-START / T-MAC-START / T-REMOTE-END・決定木・応答テンプレを明文化。Cursor ルールは §4 の実装 | v1.1 追記 |
