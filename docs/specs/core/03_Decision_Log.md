...
| P3-DG-EVENT-STG-001-2 | **`cosmic_rift` / `crown_rookery` 各 5 章** — `*_1_1`〜`*_1_5`・Boss なし（`closing_type=exit`）・日次1回は Biome 単位据置 | メイン同型の章数。最終章クリアで Biome CLEAR |
| P3-DG-EVENT-STG-001-3 | **ダック章** — 星屑の浅瀬／漂う羽の回廊／機括の巣穴／群れ鳴く裂け目／星海の最深（4/4/5/5/6F・敵Lv2→5） | 短編日次＋段階 |
| P3-DG-EVENT-STG-001-4 | **レイヴン章** — 黒羽の入口／宝冠の足場／塒の裂け枝／群れの塒／王鳥の頂（4/4/5/5/6F・敵Lv8→12） | 装備ハント向け段階 |
| P3-DG-EVENT-STG-001-5 | 章クリックで確認→突入（既存 `_on_stage_card_pressed`）。敵プール／部屋重み／日次枠は Biome `DungeonData` 継承 | 新規アセット0 |

**Closeout（2026-07-21）:** データ＋UI共通化＋unit。
