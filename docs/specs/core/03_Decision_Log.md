...
## 章クリア部隊員加入（2026-07-15 — P3-JOIN-001）

> オーナー GO（推奨一式）。SSOT=`docs/specs/decisions/03_ChapterRosterJoin.md`。`P3-D036b-9` を新進行で上書き。

| # | 決定事項 | 根拠 |
|---|---|---|
| P3-JOIN-001-1 | **開始=導入選択1人のみ**（S1）。残り基本職はロスター未所持 | 加入会話に意味を持たせる |
| P3-JOIN-001-2 | **①モーンゲート** `1-1`/`1-2`/`1-3`/`1-5` 初回クリアで次の未所持を会話加入（案C） | 序盤で顔とセリフを積む |
| P3-JOIN-001-3 | 加入キュー固定: ガレン→リーヴァ→**アイリス**→アルド→**ロアン**（選択済みスキップ） | 役割の段階解放 |
| P3-JOIN-001-4 | UI=立ち絵会話（ニーナ1行橋渡し＋本人2〜3）。スキップ可・操作講習なし | 導入方針と整合 |
| P3-JOIN-001-5 | 旧セーブ=`starter_progression_v1=false` で既加入扱い（自動補完維持） | セーブ破壊回避 |
| P3-JOIN-001b-1 | 加入立ち絵は **戦闘ドット（idle／ICO_CHR）準拠** で再生成。セリフも外見口調に合わせて改稿 | オーナー指示：ドット準拠 |
| P3-JOIN-001c-1 | 表示名改訂: **エリアス→アイリス**、**ミレイ→ロアン**（`adventurer_2`/`4`・ドット外見整合）。内部 id／固有パッシブ key は互換維持 | オーナー指示：改名 |
