...
## セーフエリア／レターボックス（2026-07-20 — P3-UI-SAFE-001）

> **オーナー GO（案B）** — iPhone 16 でアイコン等のはみ出し。Mac では問題なし。

| # | 決定 | 根拠 |
|---|---|---|
| P3-UI-SAFE-001-1 | **stretch aspect=`keep`**（旧 `expand` から変更） | 9:16 を維持し上下レターボックス。端末比差での引き伸ばし／端食い込みを抑える |
| P3-UI-SAFE-001-2 | **セーフエリア余白** — `SafeAreaHelper` が OS safe area とレターボックスの差分だけビューポート inset を算出 | ノッチ／Dynamic Island／ホームインジケータ対策。固定黒帯より機種追従 |
| P3-UI-SAFE-001-3 | **適用対象** — BottomNav／HubView 等コンテンツホスト／Header／Title Margin／Dungeon `MainVBox`。背景 Texture は全面のまま | 操作 UI のみ内側へ。見た目のフルブリードは維持 |
| P3-UI-SAFE-001-4 | **再適用** — autoload `SafeAreaPad` がシーン切替・リサイズで再計算 | 回転・マルチウィンドウでもずれない |
