...
## 調査許可コード（2026-07-20 — P3-CODE-REDEEM-001）

> **オーナー GO** — 推奨案A（シリアル→配布ボックス）・キャラ込み・マイページ・β即時。

| # | 決定 | 根拠 |
|---|---|---|
| P3-CODE-REDEEM-001-1 | **入力場所** — マイページ（隊長台帳）「調査許可コード」 | 配布ボックスと同じ画面で完結 |
| P3-CODE-REDEEM-001-2 | **成功時** — `CommanderGiftBox.enqueue` → 既存受取 UI | 受取導線を二重化しない |
| P3-CODE-REDEEM-001-3 | **オフライン辞書** — `RedeemCodeCatalog`。セーブ単位1回限り（`commander.redeemed_codes`） | β・テスター／補填向け。厳密配布は将来サーバ |
| P3-CODE-REDEEM-001-4 | **付与物** — Gold／魔晶石／素材／チケット／助っ人（未所持=加入・所持済=限界突破+1） | オーナー「キャラも含む」 |
| P3-CODE-REDEEM-001-5 | **初期コード** — `CROWNBETA`（補給）／`CROWNALLY`（ヴァルデン）／`CROWNSEER`（セリン）／`SUPPLY01`（小補給） | ハイフン・空白は正規化で同一視 |
