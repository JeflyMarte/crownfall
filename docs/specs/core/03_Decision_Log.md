...
| P3-GACHA-005-3 | **スターター分離** — `adventurer_0..4`（アルド等）は開始配布・★4表示・固有パッシブ。`gacha_helpers/` のみがプール | ガチャに基本5職を出さない |
...
| Biome | id 一覧（雑魚→Boss） |
|---|---|
| ③ | `blood_leech` / `dead_poison_frog` / `mist_mantis` / `marsh_king` / `great_claw` / `moldgar` |
| ④ | `ship_eater_crab` / `skull_turtle` / `undertaker_shark` / `samurai_fish` / `ninja_octopus` / `nereion` |
| ⑤ | `frost_claw_raptor` / `vergaron` / `storm_joe` / `oldrex` / `greios` / `eldion` |
...
## モーンゲート色違い亜種（2026-07-11 — P3-ENEMY-VAR-001）

> **オーナー GO** — 雑魚4+Elite1 の色替え亜種。ステは原種据置（出現箇所は後続調整可）。命名はオーナー確定。

| # | 決定 | 根拠 |
|---|---|---|
| P3-ENEMY-VAR-001-1 | **亜種5体** — 腐食デスハウンド / 骨灰蟲ボーンローチ / 酸水晶ニードルラット / 古文書喰いの大ネズミ / オールドクロックモス | 鉱物化適応の層差 |
| P3-ENEMY-VAR-001-2 | **アセット** — 原種シート・図鑑肖像を乗算ティント複製（`tools/generate_mourngate_variants.py`） | 作画コスト抑制 |
| P3-ENEMY-VAR-001-3 | **データ** — 独立 `EnemyData` + 図鑑別 id。drop は原種テーブル流用 | ステ未確定のまま登録 |
| P3-ENEMY-VAR-001-4 | **pool** — ハード弧専用（`enemy_pool_hard` / `elite_pool_hard`）。ノーマルは原種のみ | P3-DG-TIER-STG-001 で分離 |

## 危険度ティア×章進行（2026-07-11 — P3-DG-TIER-STG-001）

> **オーナー GO** — 同章1-1〜5-5の二周目。全局ゲート + ティア別敵プール。P3-D164 の Biome 単位 +3/+6 加算を上書き。

| # | 決定 | 根拠 |
|---|---|---|
| P3-DG-TIER-STG-001-1 | **弧** — ノーマル1-1〜5-5（全Biome）→ ハード同章 → ナイトメア同章。1-6 章追加なし | オーナー「6-1=同1-1のH版」ではなく **5-5後H1-1** |
| P3-DG-TIER-STG-001-2 | **全局解禁** — H1-1=`frostridge_5_5` ノーマルクリア / NM1-1=同ステージハードクリア | 二周目・三周目 |
| P3-DG-TIER-STG-001-3 | **弧内進行** — H1-2←H1-1、H2-1←H1-5（ノーマルと同型・ティア別クリア） | 直列 |
| P3-DG-TIER-STG-001-4 | **敵Lv** — `stage.enemy_level + {0,49,74}`。H1-1=50 > N5-5(49) | +3/+6 廃止 |
| P3-DG-TIER-STG-001-5 | **敵プール** — `enemy_pool_hard` / `elite_pool_hard`（mourngate=色違い亜種） | オーナー GO |
| P3-DG-TIER-STG-001-6 | **次Biomeノーマル解放** — 当Biome N1-5ボスのみ（P3-D164-4 維持） | 横進行 |
| P3-DG-TIER-STG-001-7 | **報酬・レア** — P3-D164 倍率据置（×1.2/×1.4 / ×1.3/×1.6） | 最小差分 |
| P3-DG-TIER-STG-001-8 | **武器ドロップLv** — ハード＝ノーマル5-5帯以上（`frostridge_5_5.enemy_level±1` 下限）／ナイトメア＝同章ハード帯以上。ティア加算は武器のみ | 敵Lvとドロップ乖離解消 |
