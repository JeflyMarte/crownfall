...
1. **獣医の手当て**を回復から **ドレイン攻撃**へ（表示名 **血還の矢**／敵 power **1.1**、自己回復＝与ダメ **50%**、CD9.5／**即時** ※旧詠唱1は誤残で撤去）  
...
| 項目 | 値 |
|---|---|
| `beast_vet_care`（表示 **血還の矢**） | `effect_type=damage`／`target_type=enemy`／power **1.1**／tags `ranged,pierce,drain,support`／CD **9.5**／cast **0**／自己回復＝与ダメ×`SKILL_DRAIN_HEAL_RATIO`(**0.5**) |
