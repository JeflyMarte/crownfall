...
| 王痕片 Help | 素材行タップで片の短い説明（極限任務への直接遷移はなし） |
| 手引き | 右上 `?`＝書籍「王痕育成とは」（特権強化型）。初回は下記 5.3 |
...
## 5.3 初回王痕片入手後の手引き（2026-09-21）

| 項目 | 内容 |
|---|---|
| トリガー | 所持片が **0→1以上**（`RoyalMarkSystem.add_shards`） |
| タイミング | **拠点に戻ってから**（解放／等級／加入などの拠点演出が空いたあと） |
| 導線 | 王痕育成画面へ**強制遷移** → 到着後に書籍手引き開始 |
| 初期キャラ | **パーティ先頭**の王痕対象（いなければ roster 先頭の対象） |
| 手引き形式 | 書籍ページ（`DungeonRouteGuideOverlay`／特権強化型）3ページ |
| フラグ | `hub_guide_royal_mark_seen`（既読）／`pending_royal_mark_guide_open`（拠点→画面キュー） |
| 再表示 | 王痕画面の「？」 |
| 既存セーブ | すでに片>0 なら既読扱い（強制遷移しない） |
| 戻る | 拠点からの強制入場時は拠点へ。通常導線はキャラ管理へ |
