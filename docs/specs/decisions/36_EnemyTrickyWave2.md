...
| ID | タイプ | 挙動 | 難度 |
|---|---|---|---|
| **T8** | 途中召集 | HP≤50% かつスキルで雑魚 **1体** 追加。**戦闘中・召喚者あたり1回**（発動時点で消費＋CD9999）。生存合計 ≤ `SWARM_SIZE_CAP`(5)。報酬は通常撃破 | 高 |
| **T10** | 沈黙 | 生存ランダム1人へ **スキル／必殺封印 5戦闘秒**（通常攻撃可・一時停止中は減らない）。封印中は頭上に **❌** | 高 |
...
|---|---|
| T10 対象 | **ランダム1人**（生存） |
| T10 時計 | **戦闘クロック**（必殺／スキルCDと同型。ポーズ除外・速度倍率あり） |
...
| T8 | **`crown_eater_rat`** | `enemy_crown_call`（summon・同種1体・戦闘中1回／CD9999） |
