...
|---|---|---|---|---|
| helper_k レノール | 2 | `{-30, 270, -20}` | `lenore_seal_echo`（封緘の切っ先） | 与ダメ **×1.12**／被ダメ **×1.10**（P3-BAL-HELPER-PASSIVE-B001） |
| helper_m シアン | 3 | `{40, 80, 60}` | `sian_silent_line`（影からの号令） | 開幕・味方全体に **empower_minor**（必殺コンボは本鼓舞より弱い追加） |
| helper_n ボルグ | 3 | `{100, 60, 100}` | `borg_gate_voice`（門前の残像） | 回避 **+18%** |
| helper_o ネリ | 2 | `{-50, 40, 30}` | `neri_waterfowl_call`（水鳥の指揮） | ペット与ダメ **×1.15**／ペット防御 **×1.05**（ミレイ×1.20未満） |
