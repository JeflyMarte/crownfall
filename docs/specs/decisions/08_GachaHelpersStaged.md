...
|---|---|---|---|---|
| helper_k レノール | 2 | `{-30, 270, -20}` | `lenore_seal_echo`（封緘の切っ先） | 与ダメ **×1.18**／被ダメ **×1.12** |
| helper_m シアン | 3 | `{40, 80, 60}` | `sian_silent_line`（影からの号令） | 開幕・味方全体に **empower** |
| helper_n ボルグ | 3 | `{100, 60, 100}` | `borg_gate_voice`（門前の残像） | 回避 **+18%** |
| helper_o ネリ | 2 | `{-50, 40, 30}` | `neri_waterfowl_call`（水鳥の指揮） | ペット与ダメ **×1.25**／ペット防御 **×1.10**（`pet_defense_mult`） |
