# 必殺の役割分担（全体バフ／攻撃／回復）

**Status:** Decision 承認済（2026-08-02 — オーナー「推奨でGo」）  
**Impl:** `P3-BAL-ULTIMATE-ROLE-001`  
**関連:** `40_UltimateAoeDebuff.md`（BT多重デバフは維持。VG全体ダメは本 Decision で上書き）  
**追記（2026-08-05）:** `grand_elixir`／`dead_eye` → `79_UltElixirDeadeye.md`  
**追記（2026-09-05）:** キャラ別必殺は `138_CharacterUniqueUltimates.md`。本表は **職フォールバック／本命寄せ** の役割。表示名・分岐必殺は `137` が正

---

## 確定（職フォールバック）

| 必殺 | 役割 |
|---|---|
| `titan_roar` | 味方全体 `guard` ＋ `empower` ＋ 挑発（`taunt`）。ガレン本命＝表示 **聖盾咆哮**（`138`） |
| `ouga_retsudan` | 敵全体・威力1.9。アルド本命＝表示 **王炎断**・出血特化へ寄せ可（`138`） |
| `grand_elixir` | 味方全体回復 **20%** ＋状態異常解除・**詠唱なし**。セリン本命 |
| `dead_eye` | 単体火力・**詠唱なし**。イヴァル本命 |
| `beast_dominion` | 敵全体多重デバフ（据置）。ミレイ本命＝表示 **毒牙の嵐**（`138`） |

